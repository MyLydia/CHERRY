...
#ifdef CONFIG_CU
void modify_iptables_RG_default_policy(int add);
//used to make sure that add and delete operatiron meet consumer and produce relation.
static int iszero=0;
#endif

#if defined(CONFIG_LUNA) && defined(CONFIG_RTK_L34_ENABLE)
#include<rtk/rtusr_rg_api.h>
extern int rtk_rg_cvlan_get(rtk_rg_cvlan_info_t *cvlan_info);
extern int rtk_rg_cvlan_add(rtk_rg_cvlan_info_t *cvlan_info);
extern int rtk_rg_cvlan_del(int cvlan_id);
extern int rtk_rg_aclFilterAndQos_del(int acl_filter_idx);
extern int rtk_rg_aclFilterAndQos_add(rtk_rg_aclFilterAndQos_t *acl_filter, int *acl_filter_idx);
#endif
...
void lbd_rg_init(void)
{
	...
	snprintf(cmd, sizeof(cmd), "echo 0x%x > /proc/rg/keep_ps_eth", ether_type);
	system(cmd);
	...
	for(i = 0 ; i < vid_size ; i++)
	{
		...
		#ifdef CONFIG_CU
			cvlan_info.memberPortMask.portmask |= (RG_get_lan_phyPortMask(0xf)|(RTK_RG_ALL_CPU_PORTMASK));
			cvlan_info.untagPortMask.portmask &= ~(RG_get_lan_phyPortMask(0xf));
		#else 
	...
	#ifdef CONFIG_CU
	#else
	acl.ingress_ethertype_mask = 0xffff;
	#endif
...
#else
	memset(&(acl.ingress_dmac_mask), 0xff, 6);
	mib_get(MIB_ELAN_MAC_ADDR, acl.ingress_smac.octet);
	memset(&(acl.ingress_smac_mask), 0xff, 6);
#endif
	...
	// init shareMeter
		...
		rtk_rg_shareMeter_set (14,3000,ENABLED); // for multicast
		system("echo 15 > /proc/rg/BC_rate_limit");
		system("echo 14 > /proc/rg/IPv4_MC_rate_limit");
		system("echo 14 > /proc/rg/IPv6_MC_rate_limit");
#else
		rtk_rg_shareMeter_set (31,300,ENABLED);  // for broadcast
		rtk_rg_shareMeter_set (30,3000,ENABLED); // for multicast
		system("echo 31 > /proc/rg/BC_rate_limit");
		system("echo 30 > /proc/rg/IPv4_MC_rate_limit");
		system("echo 30 > /proc/rg/IPv6_MC_rate_limit");
#endif
...
void modify_iptables_RG_default_policy(int add) 
{	
	//unsigned char in_policy = 1;
	//unsigned char in_enable = 0;
	if(add==1) {
	...
	if(add==1) {
		//0 means drop all out;
		printf("will drop all.\n");
		RTK_RG_ACL_IPPort_Filter_Loopback_Policy(0);
	}else {
		//1 means accept all out;
		printf("will accept all.\n");
		RTK_RG_ACL_IPPort_Filter_Loopback_Policy(1);
	}
...
{
	//printf("enter check_loopback_detect.\n");
	FILE *fp = NULL;
	int i = 0xff;
...
#ifdef CONFIG_CU		
			if(iszero==0) {
				printf("will add loopback rule.\n");
				modify_iptables_RG_default_policy(1);
				iszero++;
			}
		}else {
			
			if(iszero==1) {
				printf("will delete loopback rule.\n");
				iszero--;
				modify_iptables_RG_default_policy(0);
			}
#endif
		}
		fclose(fp);
