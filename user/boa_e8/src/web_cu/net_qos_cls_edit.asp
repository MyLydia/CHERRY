...
var classTypes = new Array("一般模式", "应用类型");
var connTypes = new Array("", "TR069_INTERNET", "INTERNET", "TR069", "Other", "VOICE", "TR069_VOICE", "VOICE_INTERNET", "TR069_VOICE_INTERNET", "SPECIAL_SERVICE_1", "SPECIAL_SERVICE_2", "SPECIAL_SERVICE_3", "SPECIAL_SERVICE_4");
var iffs = new it_nr("");
...
var sel1 = new Array("phypt", "proto", "dscp", "tos", "sip", "smsk", "spts", "spte", "dip", "dmsk", "dpts", "dpte", "smac", "dmac",
...
function on_submit() 
{
	with ( document.forms[0] )	{
		...
		//condition check
		if (phypt.value==0 && proto.value==0 && dscp.value==0 && (tos.value == "") && (sip.value=="" || 
	...
	//	}
		...
		if(tos.value<0||tos.value>255) {
			tos.focus();
			alert("不正确的TOS值(1-255)");
			return;
		} else {
			sbmtstr = sbmtstr+"&matchtos="+tos.value;
		}
...
function classtypeChange()
{
	if(document.forms[0].classtype.value == 0){	
		...
		phypt.value=0;proto.value=0;dscp.value=0;tos.value="";prio.value=0;
			...
			<form id="form" action="/boaform/admin/formQosRuleEdit" method="post">
				...
				<div id='ipprotbl' style="display:none">						
					...
					<tr><td>如果WAN口802.1p被启用,则设置802.1p值: </td><td><select id="m1p" size="1"></select></td></tr>
										<br>
					<tr><td colspan="2">&nbsp; </td></tr>
					<tr><td colspan="2"><b>分类模式</b></td></tr>
					<tr><td>模式选择: </td><td><select id="classtype" size="1" style="width:200px " onChange="onchange_sel1()" name="classtype"></select></td></tr>
				</table>		<br>
				...
				<div id='appbasedlist' style="display:block;">
				<table cellSpacing="1" cellPadding="0" border="0">	
					<tr><td width="260px">连线类型:</td><td><select id="conntype" size="1" style="width:200px " name="connTypes"></select></td></tr>					
				</table>
				</div>

				<div id='normallist' style="display:block;">
				<table cellSpacing="1" cellPadding="0" border="0">
					...
					<tr><td width="260px">Type of Service(TOS)核对:</td><td><input type="text" id="tos" size="3" maxlength="3" style="width:200px " onChange="onchange_sel1();"></td></tr>
