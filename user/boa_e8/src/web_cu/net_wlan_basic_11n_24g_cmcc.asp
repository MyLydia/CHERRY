...
var ssid2_only_enable = "<% checkWrite("ssid2_only_enable"); %>";
var ssidprefix_enable = "<% checkWrite("ssidprefix_enable"); %>";
...
var _wapi_auth=new Array();
var _wapi_auth_pskForm=new Array();
var _wapi_auth_pskVal=new Array();
...
{	
	...
	get_by_id("show_wapi_cipher").style.display = "none";
	get_by_id("show_wapi_psk1").style.display = "none";
	get_by_id("show_wapi_psk2").style.display = "none";
    get_by_id("show_wapi_cert").style.display = "none";
	...
	if (security.value == 1) {	/* WIFI_SEC_WEP */
		...
		show_wpa_settings();
	}else if(security.value == 8){	/* WAPI-PSK */
			get_by_id("setting_wapi").style.display ="";
            get_by_id("show_wapi_cipher").style.display = "";
		    wapiauthtypechange();	
    }
}
...
function postSecurity(encrypt, enable1X, wpaAuth, wpaPSKFormat, wpaPSK, rsPort, rsIpAddr, rsPassword, uCipher, wpa2uCipher, wepAuth, wepLen, wepKeyFormat,wapi_auth,wapi_auth_pskForm,wapi_auth_pskVal) 
{	
...
//		document.wlanSetup.security_method.value = encrypt;	/* WIFI_SEC_NONE or WIFI_SEC_WEP */
	...
	document.wlanSetup.wapiAuth.value = wapi_auth;
    wapiauthtypechange(); 
	document.wlanSetup.wapipskFormat.value = wapi_auth_pskForm;
	document.wlanSetup.wapipskValue.value = wapi_auth_pskVal;
...
function SSIDSelected(index)
{
	...
	postSecurity(_encrypt[index], _enable1X[index],
		...
		_wepLen[index], _wepKeyFormat[index], _wapi_auth[index],_wapi_auth_pskForm[index],_wapi_auth_pskVal[index]);
	document.wlanSetup.wpaSSID.length = 0;

	if(ssidIdx==0)
	{
		document.wlanSetup.wpaSSID.options[0] = new Option("SSID1", "0", false, false);
		if(ssid2_enable==1||ssid2_only_enable==1)
		{
...
function isIncludeInvalidCharNew(str)
{
	var unsafe = 0;
	var idx;
	
	for (idx = 0; idx < str.length; idx++) {
		unsafe = str.charCodeAt(idx)<=47 || ((str.charCodeAt(idx)>=58)&&(str.charCodeAt(idx)<=64))
		||((str.charCodeAt(idx)>=91)&&(str.charCodeAt(idx)<=96))||str.charCodeAt(idx)>=123;

		if(str.charCodeAt(idx)>19967 && str.charCodeAt(idx)<40870 || str[idx]=='-' || str[idx]=='_') 
			unsafe = 0;
...
function on_init()
{
	...
	{
		get_by_id("ssid_name").style.display = "";
	}
	else
	{
		get_by_id("ssid_name").style.display= "none";
		get_by_id("ssid_namep").style.width = "44%";
	}
	
	get_by_id("control-band-id-1").style.display = "none";
	get_by_id("control-band-id-2").style.display = "";
	//get_by_id("control-band-id-1").style.display = "";
	//get_by_id("control-band-id-2").style.display = "none";
}
...
function wlSecCbClick(cb) 
{
	...
	if(status)
		document.wlanSetup.pskValue.value = _wpaPSK[document.wlanSetup.wlSsidIdx.selectedIndex];
}
...
function on_submit() 
{
	var str, band, basicRate = 0, operRate = 0, num, PrefixNum = 0;
	...
	if (document.wlanSetup.wlanEnabled.checked) {
		...
		if (calstrNum(str) > 32 - PrefixNum) {
			alert('SSID "' + str + '" 字符总和不能大于32个字符。');
			return;
		}

		/*if (isIncludeInvalidCharNew(str)) {
	...
	if (form.security_method.value == 1) {	/* WIFI_SEC_WEP */
		...
		if(document.wlanSetup.wlSsidIdx.selectedIndex == 0 && document.wlanSetup.wlanEnabled.checked)
	...
	if(ssid_idx_now == 0 && document.wlanSetup.disableWPS.checked == true){ //check only for root ssid
		if(saveChangesWPS(document.wlanSetup) == false)
			return false;
	}
...
function get_rate(curRate)
{
	...
	var rateNameLength = rate_name.length;
	if(getSelectVal('security_method') == 1)
		rateNameLength -= 16;
	
	for (i = 0; i < rateNameLength; i++) {
		if (rate_mask[i] & mask) {
			rate = (i == 0) ? 0 : 1 << (i - 1);
...
function showcontrolsideband_updated(band, idx_value)
{
	...
	{
			document.wlanSetup.ctlband.options[idx++] = new Option("上偏", "0", false, false);
			document.wlanSetup.ctlband.options[idx++] = new Option("下偏", "1", false, false);
...
function update_wps(form, enable)
{
	if(enable){
		enableCheckBox(form.elements["disableWPS"]);
...
function update_ssid_info()
{
	with (document.wlanSetup) {
		...
		if(wlSsidIdx.selectedIndex == 0)	 //SSID1
		{
			...
			update_wps(document.wlanSetup, 1);
		}
		else                                 //SSID2
		{
			ssid_idx_now = 2;
			...
			update_wps(document.wlanSetup, 0);
		}
	}
}
...
{
    if(get_by_id("security_method").value == 8){
	var passwordtx = getElById('wapipsktx'),  password = getElById('wapipskValue');
	...
	var passwordtx = getElById('wpapsktx'),  password = getElById('wpapsk');
	passwordtx.value = password.value;
	password.style.display = 'none';
	passwordtx.style.display = 'inline';
	password.focus();
...
var wps_warn_msg3="PIN 已产生, 请按下确定钮来储存设定及套用变更.";

function genPinClicked(form)
{
	var num_str="1";
	var rand_no;
	var num;
	...
	num = parseInt(num, 10);	
	form.elements["localPin"].value = num; 
	alert(wps_warn_msg3);
}
...
function saveChangesWPS(form)
{
	...
	if (ret == 1) {
		alert('PIN 的长度无效!设备 PIN 通常是八个数字长。');
		form.localPin.focus();
		return false;
	}
	else if (ret == 2) {
		alert('PIN 无效!设备 PIN 必须是数字。');
		form.localPin.focus();		
		return false;
	}
	else if (ret == 3) {
...
<body topmargin="0" leftmargin="0" marginwidth="0" marginheight="0" alink="#000000" link="#000000" vlink="#000000" onload="BandSelected(0)">
	...
	<table border="0" cellpadding="2" cellspacing="0"  width="420">
		<tr>
			<td id="ssid_namep" width="44%">SSID名称:</td>
			<td id="ssid_name" width="0%">CU_</td>
			<td><input size="10" style="WIDTH:180px;height:30px;" name='ssid' maxlength="32" value=""></td>
		</tr>
	</table>
	...
	<table border="0" cellpadding="4" cellspacing="0" width="420">
		...
		<tr>
			<td width="43%" id="control-band-id-1">控制边带:</td>
			<td width="43%" id="control-band-id-2">扩展信道:</td>
	...
	<table border="0" cellpadding="4" cellspacing="0" width="400">
		...
		<tr>
			<td>Beacon信标帧发送间隔:</td>
			<td colspan="2">
			<input size="10" style="WIDTH: 80px" type='text' name="beaconInterval" size="10" maxlength="4">
			(范围: 20~1024)
			</td>
		</tr>
		<tr>
			<td>DTIM间隔:</td>
			<td colspan="2">
			<input size="10" style="WIDTH: 80px" type='text' name="dtimPeriod" size="5" maxlength="3">
			(范围: 1~255)
			</td>
		</tr>		
		<tr>
	...
	<table border="0" cellpadding="0" cellspacing="0" width="400">
		...
		<tr id="setting_wapi" style="display:none">
			<td colspan="2" width="100%">
				<table width="400" border="0" cellpadding="4" cellspacing="0">
					<tr id="show_wapi_cipher" style="display:none">
						<td width="45%">加密/认证配置(WAPI):</td>
						<td colspan="2"><select style="width: 180px" size="1" name="wapiAuth" id="wapiAuth" onchange="wapiauthtypechange()">
							<option value=1>WAPI-Cert</option>
							<option value=2 selected="selected">WAPI-PSK</option>
						</select></td>
					</tr>				
				</table>
				<table width="400" border="0" cellpadding="4" cellspacing="0">
					<tr id="show_wapi_psk1" style="display:none">
						<td width="45%">PSK FORMAT(WAPI):</td>
						<td colspan="2"><select style="width: 180px" size="1" name="wapipskFormat" id="wapipskFormat">
				...
				<table width="500" border="0" cellpadding="4" cellspacing="0">				
					<tr id="show_wapi_psk2" style="display:none">
						<td width="31%">WAPI 共享密钥:</td>
						<td width="55% colspan="2">
						<input size="10" style="WIDTH: 180px" type="password" name="wapipskValue" id="wapipskValue" maxlength="27" value="">
						<input size="10" style="WIDTH: 180px;display: none;" type="text" name="wapipskValuetx" id="wapipsktx" maxlength="27" value="">
						<input class="btnsaveup" type="button" id="showpasswd1" name="showpasswd1" value="显示密码" onmousedown="showPassWord();" onmouseup="hidePassWord();">
						</td>
					</tr>
					<tr id="show_wapi_cert" style="display:none">
						<td width="31%">WAPI 证书认证:</td>
						<td width="55% colspan="2">
						<input class="btnsaveup" type="button" id="getwapicert" name="getwapicert" value="获取证书" onclick="javascrtpt:window.location.href='/bd/wapicert.asp'">
						</td>
					</tr>
				</table>
			</td>
		...
		<tr id="setting_wps">
			<td colspan="2" width="100%">
				...
				<table width="500" border="0" cellpadding="4" cellspacing="0">
					<tr id="show_wps_pin" style="display:none">
						<td width="31%">WPS PIN:</td>
						<td width="55% colspan="2">
						<input type="text" name="localPin" size="10" style="width: 180px" maxlength="10" onkeypress="return noenter(event)" value=<% getInfo("wscLoocalPin"); %>>
						<input class="btnsaveup" type="button" value="重新产生PIN" name="genPIN" onClick="return genPinClicked(document.wlanSetup)">
					</tr>
				</table>
			</td>
		</tr>
