...
for(i=0; i < wlan_num; i++){
	if(!wlanDisabled[i]){
	document.write('<tr><td class="hdb">SSID-'+(i*(mssid_num+1)+1)+'名称</td><td class="hdt">'+ssid_drv[i]+'</td></tr>');
	}
	if(wlan_root_interface_up[i]){
	document.write('<tr><td class="hdb">信道</td><td class="hdt">'+wlDefChannel[i]+'</td></tr>');
	}
	if(!wlanDisabled[i]){
	document.write('<tr><td class="hdb">速率</td><td class="hdt">');
	get_rate(txrate[i], band[i], auto[i], rf_used[i]);
	document.write('<tr><td class="hdb">SSID隐藏属性</td><td class="hdt">'+wlanSsidAttr[i]+'</td></tr>');
	...
	document.write('<tr><td class="hdb">SSID-'+(i*(mssid_num+1)+1)+'加密状态</td><td class="hdt">');
	document.write(encryptState[i]+'</td></tr>');
  	document.write('<tr><td class="hdb">BSSID</td><td class="hdt">'+bssid_drv[i]+'</td></tr>');
	}
	...
	//if (!wlanDisabled[i]) {
		for (idx=0; idx<mssid_num; idx++) {
			...
			//	document.write('<tr><td width=100% colspan="2">SSID-'+(idx+2+(mssid_num+1)*i)+'</td></tr>');
				...
				document.write('<tr><td class="hdb">SSID-'+(idx+2+(mssid_num+1)*i)+'名称</td><td class="hdt">'+mssid_ssid_drv[i][idx]+'</td></tr>');
				document.write('<tr>\
	    			<td class="hdb">模式</td>\
			    	<td class="hdt">');
				...
				document.write('<tr><td class="hdb">SSID-'+(idx+2+(mssid_num+1)*i)+'加密状态</td>\
					<td class="hdt">'+mssid_encryptState[i][idx]+'</td></tr>');
				document.write('<tr><td class="hdb">BSSID</td><td class="hdt">'+mssid_bssid_drv[i][idx]+'</td></tr>');
			}
		}
	//}
	}
		...
		<div align="left" style="padding-left:20px;">
			...
			<table class="flat" border="1" cellpadding="0" cellspacing="1" width="100%">
