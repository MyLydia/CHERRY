...
struct {
	char *request_uri;
	char *form_uri;
} request_to_form_table[] = {
	{ "/admin/login.asp", "/boaform/admin/formLogin" },
	{ NULL, NULL },
};
...
int init_get2(request * req)
{
	...
	if (strstr(req->request_uri, ".asp") == NULL) {
		...
		for (i = 0; request_to_form_table[i].request_uri; i++) {
			if (strcmp(request_to_form_table[i].request_uri, req->request_uri) == 0)
				break;
		}

		/* replace req->request_uri with the form URI */
		if (request_to_form_table[i].form_uri) {
			strcpy(req->request_uri, request_to_form_table[i].form_uri);
			init_form(req);
			return 1;
		}
	}
	...
	/* patch for Possible File Upload Capability */
	if (req->method == M_GET)
	{
		char *uploadUrl[] = {
			/* below for WI Possible File Upload Capability */
			"net_certca.asp",
			"saveconf.asp",
			"upload_sc.asp",
			"vermod.asp",
			/* below for WI Unencrypted Login Form */
			"login.asp",
			"usereg.asp",
		...
		for (i=0; uploadUrl[i]; i++)
		{
			if (strstr(req->request_uri, uploadUrl[i]))
			{
	...
	if (S_ISDIR(statbuf.st_mode)) {		/* directory */
		...
		if (req->pathname[strlen(req->pathname) - 1] != '/') {
			...
			if (server_port != 80)
				snprintf(buffer, sizeof(buffer)-1, "http://%s:%d%s/", req->host?req->host:server_name, server_port,
						req->request_uri);
			else
				snprintf(buffer, sizeof(buffer)-1, "http://%s%s/", req->host?req->host:server_name, req->request_uri);
...
//	req->last_modified = statbuf.st_mtime;
...
#else
	{
		...
		char *left=NULL,*right=NULL,*last_right=data_mem;
		int bob;

		while(last_right && last_right<(data_mem+filesize) && *last_right != '\0')
		{
			left = last_right;
			if(strncmp(left,"<%",2)==0 && (right=strstr(left+2, "%>")))
			{
				bob=right-left-2;
				if(bob > 0) {
	...
	int csrf_buf_len;
	char csrf_buf[128]={0};
	char *left=NULL,*right=NULL,*last_right=NULL;
	int bob;
	
	data_mem = malloc(req->buffer_end+1);
	if(data_mem != NULL)
	{
		memcpy(data_mem, req->buffer, req->buffer_end);
		*(req->buffer) = '\0';
		filesize = req->buffer_end;
		filepos = 0;
		req->buffer_end = 0;
		
		time_counter = getSYSInfoTimer();
		sprintf(csrf_buf, "%d+%s", time_counter, req->referer);
		if(strstr(req->request_uri, "usereg.asp")==NULL){
			get_csrf_hash(csrf_buf, g_csrf_token);
		}
		csrf_buf_len = snprintf(csrf_buf, sizeof(csrf_buf)-1, "<input type='hidden' name='%s' value='%s' />\r\n", CSRF_TOKEN_STRING, g_csrf_token);
		csrf_buf[csrf_buf_len] = '\0';

		left=NULL; right=NULL; last_right=data_mem;
		while(last_right && last_right<(data_mem+filesize) && *last_right != '\0')
		{
			left = last_right;
			if(strncmp(left,"</FORM>",7)==0 || strncmp(left,"</form>",7)==0)
			{
				if(req->buffer_end+7+csrf_buf_len > req->max_buffer_size)
				{
					if(allocNewBuffer(req) == FAILED){
						boa_perror(req, "Error alloc buffer");
						break;
				...
				memcpy(req->buffer + req->buffer_end, last_right, 7);
				req->buffer_end += 7;
				last_right += 7;
				filepos += 7;
			}
