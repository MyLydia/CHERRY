...
extern int attack_check(char *buf);
#endif
...
struct formPrivilege formPrivilegeList[] = {
	...
	{"formUpgradeRedirect", PRIV_USER},
#endif
	{"formSaveConfig", PRIV_ENG},
#ifdef CONFIG_RTL_WAPI_SUPPORT
	{"formSaveWapiCert", PRIV_ENG},
#endif
...
#if CONFIG_USER_BOA_CSRF
	if(ret < 0 && req->content_type != NULL && strcmp(var, CSRF_TOKEN_STRING) == 0){
		if(strstr(req->content_type, "multipart/form-data")) {
			memset(query_temp_var, 0 ,MAX_QUERY_TEMP_VAL_SIZE);
			char* start = NULL;
			char* pos = NULL;
			char* end = buf + strlen(buf);
			//unsigned char csrf_token[32] = {0};
			int i=0;
			
			start = strstr(buf, CSRF_TOKEN_STRING);
			if(start){
				start = start + strlen(CSRF_TOKEN_STRING);
				pos = start;
				//printf("start =%s\n", start);
				//find csrf value
				while(pos < end){
					if(isxdigit(pos[0])){
						query_temp_var[i] = pos[0];
						i++;
						if(i >= MAX_QUERY_TEMP_VAL_SIZE)
							break;
					}
...
int boaRedirect(request *req,char *url)
{
	...
	{
		//url is http://192.168.1.1:8080/xxx.asp
		char *link = url+7;
		if((link=strstr(link, "/"))!=NULL)
		{
			url = link;
		}
	}
...
int allocNewBuffer(request *req)
{
	char *newBuffer = NULL;
#if 1
	newBuffer = (char *)realloc(req->buffer, req->max_buffer_size*2+1);
	if(newBuffer == NULL){
		printf(">> [BOA] cannot aclloc memory, size = %d\n", req->max_buffer_size*2+1);
		return ERROR;
	}
...
void handleForm(request *req)
{
	...
	char reguserform[128]={0};
#define FORM_USER_REG_NAME "formUserReg"
#endif
...
//	printf("into handleForm %s........\n",req->request_uri);
	...
	else
	{
		...
		now_form=root_form;
//nessus cause WAN reconnect because it will apply usereg.asp
#if defined(CONFIG_YUEME)
		sprintf(reguserform, "%s%s", myalias, FORM_USER_REG_NAME);
		if(req->method==M_GET && strstr(req->request_uri,reguserform)){
			send_r_bad_request(req);
			return;
		}
#endif
#if !defined(CONFIG_CMCC) && !defined(CONFIG_CU)
		/* patch for webinspect Admin Section Must Require Authentication */
		if (req->method == M_GET)
		{
			if (strstr(req->request_uri, "formLogin"))
			{
				...
				for (j=0; j<req->cgi_env_index; j++)
				{
					if (strcasestr(req->cgi_env[j], "X_SCAN_MEMO") && strcasestr(req->cgi_env[j], "Crawl"))
					{
						send_r_bad_request(req);
						return;
					}
		...
		/* patch for Admin Section Must Require Authentication */
		if (req->method == M_GET)
		{
			/* forbid such get request: GET 192.168.1.1:8080/boaform/admin/formEthernet */
			if (!strstr(req->request_uri, ".asp") && !strstr(req->request_uri, ".cgi") && !strstr(req->request_uri, ".html") && 
				(NULL == req->query_string))
			{
				send_r_bad_request(req);
				return;
			}
		...
		/* patch for Cross-Site Scripting issue */
		if ((req->method == M_GET) && (req->query_string != NULL))
		{
			if (attack_check(req->query_string))
			{
				send_r_bad_request(req);
				return;
			}
			/* TODO: 
			 * attack_check is created for attack filter, but I am not sure if attack_check is too strict for all
			 * case test before. so reserve below redundant code now
			 * I think below code should be removed later, If you see print info, then we should add "script" or "alert"
			 * keyword to hitKeyword array.
			 */
			if (strcasestr(req->query_string, "script") || strcasestr(req->query_string, "alert"))
			{
				printf("=========>>>> warning! please add script or alert to attack hitKeywork array!!!!!!!!\n");
				send_r_bad_request(req);
				return;
			}
		}
		...
		if ((req->method == M_POST))
		{
			char *buf, *chkptr, *tmpStr, tmpChar, *dst;
			char *translatedPostTblBuf=NULL;
			unsigned int bufSize;
			struct stat statbuf;
			int i, postTblLen=0;
			int postSecurityFlagValue;
			unsigned int csum = 0;
			
			fstat(req->post_data_fd, &statbuf);
			//form table size maybe larger than 4096
			//bufSize = (statbuf.st_size > 4096)?4096:statbuf.st_size;
			...
			lseek(req->post_data_fd, SEEK_SET, 0);
			read(req->post_data_fd, buf, bufSize);
			buf[bufSize]=0;
			i = bufSize - 1;
			while (i > 0)
			{
			...
			chkptr = strstr(buf, "postSecurityFlag");
			if (chkptr != NULL)
			{
				/* Get the value of postSecurityFlag */
				tmpStr = boaGetVar(req, "postSecurityFlag", "");
				postSecurityFlagValue = strtoul(tmpStr, NULL, 10);

				/* truncate the post table for further checking */
				chkptr[0] = 0;
				//printf("truncate POST:\n%s\n", buf);
				dst = buf;
				postTblLen = strlen(buf);
				//printf("translated(%d):\n%s\n", postTblLen, dst);
...
CSUM_CHECK:
				...
				//printf("postSecurityFlagValue=%d chkValue=%d\n", postSecurityFlagValue, csum);
				if (postSecurityFlagValue != csum)
				{
#ifdef LOGIN_ERR_TIMES_LIMITED
					struct errlogin_entry * pErrlog_entry = NULL;
#endif
					if (NULL == translatedPostTblBuf)
					{
						csum = 0;
						//form table size maybe larger than 4096
						//translatedPostTblBuf = (char *)malloc(4097);
						translatedPostTblBuf = (char *)malloc(bufSize+1);
						if (NULL == translatedPostTblBuf)
						{
							send_r_bad_request(req);
							return;
						}
						memset(translatedPostTblBuf, 0, bufSize+1);
						postTblLen = getPostTableForValidityCheck(buf, translatedPostTblBuf);
						//printf("translatedPostTblBuf:\n%s\n", translatedPostTblBuf);
						dst = translatedPostTblBuf;

						goto CSUM_CHECK;
					}
BAD_REPLY:				
#ifdef LOGIN_ERR_TIMES_LIMITED
					if(!strcmp(req->request_uri,"/boaform/admin/formLogin"))
					{
						...
						if(pErrlog_entry){
							...
							{	/* free login account after error login count reaches MAX_LOGIN_NUM */
							...
							strncpy(pErrlog_entry->remote_ip_addr, req->remote_ip_addr, sizeof(pErrlog_entry->remote_ip_addr));
							pErrlog_entry->next = errlogin_list;
							errlogin_list = pErrlog_entry;
						}
						//printf("checksum error: %s count[%d] time[%lld]\n",req->request_uri,pErrlog_entry->login_count,pErrlog_entry->last_time);
					}				
#endif
					free(buf);
					if (translatedPostTblBuf != NULL)
						free(translatedPostTblBuf);
					send_r_bad_request(req);
					return;
				}
			}
			else
			{
				...
				{
					//this is file upload post, skip validity check.
				}
				else
				{
					printf("WARNING! postSecurityFlag not exist in the page!\n%s\n", buf);
					//free(buf);
					//send_r_bad_request(req);
					//return;
					goto BAD_REPLY;
		...
		if(req->method==M_POST && skipFlag==0){
			...
			if (!str[0] && g_csrf_token[0] !=0 ) {//page does not contain csrf token, but the web page refeshed before
				...
				if(strcmp(str, g_csrf_token)!=0){//the csrf token is not matched
					//printf("[%s:%d]\n",__func__,__LINE__);
					//printf("%s=%d, g_csrf_token=%s\n",CSRF_TOKEN_STRING, g_csrf_token);
					//send_r_forbidden(req);
					str = boaGetVar(req, "submit-url", "");
					boaRedirect(req, str);
					send_r_request_ok(req);
...
void handleScript(request * req, char *left, char *right)
{
	...
	if(strncmp(left,"<%",2)==0)left += 2;
	if(strncmp(right-2,"%>",2)==0)right -= 2;
	while (left < right) {
		left += strspn(left, " ;(),");
		if (left >= right)
			break;
