 ...
 *
 * $Id: nls.c,v 1.2 2005/08/29 08:13:52 kaohj Exp $
 */
...
int nls_try_redirect(request * req)
{
  ...
  while (current) {
      ...
      if (current->fakename[current->fake_len - 1] != '/' &&
			...
			if (server_port!=80)
				snprintf(buffer, sizeof(buffer)-1, "http://%s:%u",req->host?req->host:server_name,
						server_port);
				else
					snprintf(buffer, sizeof(buffer)-1, "http://%s",req->host?req->host:server_name);
