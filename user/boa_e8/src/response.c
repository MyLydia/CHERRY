...
//Chrome will leave FIN_WAIT_2 in netstat -ano in PC, need close socket to free it. IE didn't have this problem
...
void print_set_cookie(request * req)
{
	...
	if(get_sessionid_from_cookie(req, sessionid))
		sprintf(buf, "sessionid=%s; Path=/; ", sessionid);
	else {
		time_counter = getSYSInfoTimer();
		sprintf(buf, "sessionid=%lld; Path=/; ", time_counter);
	}
...
void send_redirect_temp(request * req, char *url)
{
	...
	if(strstr(url, "http")) {
		...
		sscanf(url, "%*[http|https]://%n%*[^/]%n%*s", &n, &n1);
		if(*(url+n) == '[') 
		{
			n++;
			len = sizeof(address)-1;
			len = ((n1-n) > len)?len:(n1-n);
			memset(address, 0, sizeof(address));
			strncpy(address, url+n, len);
			tmp_url = strchr(address, ']');
			if(tmp_url) *tmp_url = '\0';
		}
		//sscanf(address, "%*[[]%[^]]", &address);
		if(address[0] && inet_pton(PF_INET6, address, &addr6)) 
		{
			if(n1 > 0 && *(url+n1) != '\0' && *(url+n1+1) != '\0'&& n1 < sizeof(e_s))
			{
				strncpy(e_s, url, n1);
				_escape_string(url+n1, e_s+n1, sizeof(e_s)-n1-1);
				tmp_url = e_s;
			}
			else tmp_url = url;
		}
		else{
			tmp_url = escape_string(url, e_s);
		}
	}
	else{
		tmp_url = escape_string(url, e_s);
...
void send_r_entity_too_large(request * req)
{
	...
	if (req->method != M_HEAD) {
		req_write(req, "<HTML><HEAD><TITLE>413 Request Entity Too Large</TITLE><META http-equiv=content-type content=\"text/html; charset=gbk\"></HEAD>\n"
					 "<BODY>\n<script type=\"text/javascript\">setTimeout('countdown()', 1000);\n"
					"function countdown() {\n"
					"var s = document.getElementById('timer');\n"
					"s.innerHTML = s.innerHTML - 1;\n"
