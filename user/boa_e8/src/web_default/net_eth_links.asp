...
var cgi = new Object();
var links = new Array();
var lan_interface_num = <%checkWrite("lan_interface_num") %>;
var wlan_interface_num = <%checkWrite("wlan_interface_num") %>;
...
function on_linkchange(itlk)
{
	with ( document.forms[0] ) 
	{
		if(itlk == null)
		{
			...
			//checkbox array
			for(var i = 0; i < lan_interface_num; i ++) chkpt[i].checked = false;
		...
		else
		{
			...
			{
				var mask=0, mask_wlan0=0, mask_wlan1=0;
				for(var idx=0; idx<wlan_interface_num; idx++)
					mask |= (1<<idx);
				mask_wlan0 = (mask<<4);
				mask_wlan1 = (mask_wlan0<<wlan_interface_num);
				var tmp = ((itlk.itfGroup & mask_wlan0) << wlan_interface_num) | ((itlk.itfGroup & mask_wlan1) >>> wlan_interface_num);
...
//1851 #define CHANNEL_MODE_DSLITE     6
			...
			if ((1 == lkmode.value) && (6 == applicationtype.options.length))
			{
...
function is_configurable()
{
	...
	if(cwmp_configurable == 0 && lk != "new" && links[lk].name.search("TR069") != -1)
		return false;
		if(Procfg=="1"){
		if(cwmp_configurable == 0 && lk != "new" && links[lk].name.search("VOICE") != -1)
			return false;
		if(cwmp_configurable == 0 && lk != "new" && links[lk].name.search("Other") != -1)
			return false;
	}	
	return true;
}
...
function on_ctrlupdate(obj)
{
	...
	{		
		if(obj.name == "lkname")
		{
			...
			if(Procfg=="1"){
				mtu.value = 1460;
				document.getElementById('mtu').readonly = true;
			}
...
//star:20090302 START ppp username and password can be empty
			...
			if (ipmode[2].checked == true  && lkmode.value == 1)
			{
				if( (pppUsername.value == "" || pppPassword.value == "")){
						alert("密码与用户名不可为空，请重新输入！");
						return;
				}
				else
				{
					if(pppUsername.value!="" && sji_checkpppacc(pppUsername.value, 1, 30) == false)
					{
						pppUsername.focus();
					...
					{
						pppPassword.focus();
						alert("新建连接的pppoe密码不能全为星号，请重新输入!");
						return;
					}
				}
		...
		//	cmodeV6.value = 0;
			...
			if(wlan_interface_change){
				...
				mask_wlan0 = (mask<<4);
				mask_wlan1 = (mask_wlan0<<wlan_interface_num);
				var tmp = (((ptmap & mask_wlan0) << wlan_interface_num) | ((ptmap & mask_wlan1) >>> wlan_interface_num));
				ptmap = (tmp | (ptmap & 0xf));
			}
...
*/						
		...
		postTableEncrypt(document.forms[0].postSecurityFlag, document.adsl);
		...
		<DIV align="left" style="padding-left:20px; padding-top:5px">
			<form id="form" action="/boaform/admin/formEthernet" method="post" name="adsl">
				...
				<table id="tbmtu" cellpadding="0px" cellspacing="2px">
					<tr class="sep"><td colspan="2"><hr align="left" class="sep" size="1" width="100%"></td></tr>
					<tr nowrap><td width="150px">MTU：</td><td><input type="text" id="mtu" name="mtu" maxlength="4"  size="4" style="width:40px "></td></tr>
				</table>
				<table id="tbdial" cellpadding="0px" cellspacing="2px">
					<tr class="sep"><td colspan="2"><hr align="left" class="sep" size="1" width="100%"></td></tr>
					<tr nowrap><td width="150px">用户名：</td><td><input type="text" name="pppUsername" maxlength="30" size="16" style="width:150px "></td></tr>
					<tr nowrap><td>密码：</td><td><input type="password" name="pppPassword" autocomplete="off" maxlength="30" size="16" style="width:150px "></td></tr>
				...
				<table id="tbqos" cellpadding="0px" cellspacing="2px">
					...
					<tr nowrap><td width="150px">关闭LAN侧DHCP：</td><td><input type="checkbox" name="disableLanDhcp"></td></tr>
				</table>
				...
				<div id=tbdnsv6set style="display:block">
				<table border=0 width="600" cellspacing=4 cellpadding=0>
					<tr class="sep"><td colspan="2"><hr align="left" class="sep" size="1" width="100%"></td></tr>
					<tr nowrap>
						<td width="150px">Request DNSv6:</td>
						<td><input type="radio" value="1" name="dnsv6Mode" checked onClick='dnsv6ModeClicked()'>Enable</td>
					</tr>
					<tr nowrap>
						<td width="150px"></td>
						<td><input type="radio" value="0" name="dnsv6Mode" onClick='dnsv6ModeClicked()'>Disable</td>							
					</tr>
					<tr nowrap>		
						<td width="150px">首选DNS:</td>
						<td><input type="text" name="Ipv6Dns1" size="18" maxlength="15" value= ></td>
					</tr>
					<tr nowrap>
						<td width="150px">备选DNS:</td>
						<td><input type=text name="Ipv6Dns2" size="18" maxlength="15" value=></td>
					</tr>
				</table>
				</div>
				...
				<input type="hidden" name="submit-url" value="/net_eth_links.asp">
