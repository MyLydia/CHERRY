...
var wlan_ssid_num = <% checkWrite("wlan_ssid_num"); %>;
var ssid_name = <% checkWrite("wifi_timer_ssid_name"); %>;
...
function convertSSID(SSIDMask)
{
	var value="";
	var bit_shift;
	for(i=0;i<wlan_ssid_num*wlan_num;i++){
		if(i>=wlan_ssid_num)
			bit_shift = (i-wlan_ssid_num+16);
		else
			bit_shift = i;
		if(SSIDMask & (1<<bit_shift))
			value+=ssid_name[i];
		else
			value+="*";
		value+=" ";
...
function on_submit(edit) 
{
	if(pageCheckValue()){
		...
		postTableEncrypt(document.forms[0].postSecurityFlag, document.wlanschedule);
...
function Modify(i)
{
	...
	for (var j=0; j<wlan_ssid_num*wlan_num; j++) {
		if(j>=wlan_ssid_num)
			bit_shift = (j-wlan_ssid_num+16);
		else
			bit_shift = j;
		document.forms[0].chkpt[j].checked = (_SSIDMask[i] & (0x1 << bit_shift));
	}
...
function pageCheckValue()
{
	...
	if(document.forms[0].chkpt[j].checked == true)
			getObj("ssid_mask").value |= (1<<bit_shift);
	}
