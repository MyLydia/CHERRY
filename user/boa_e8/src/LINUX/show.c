...
int main(int argc, char *argv[])
{
	...
	if ( !strcmp(argv[argNum], "system") ) {
		...
		if ( !strcmp(argv[argNum], "version")) {
			...
			FILE * fp;
			char buf[256]={0};
			mib_get( MIB_HW_CWMP_MANUFACTURER, (void *)buffer);
			printf("Manufacturer: %s\n",buffer);
			mib_get( MIB_HW_CWMP_PRODUCTCLASS, (void *)buffer);
			printf("Productclass: %s\n",buffer);
			mib_get( MIB_HW_HWVER, (void *)buffer);
			printf("Product Version: %s\n",buffer);			
			mib_get( MIB_HW_PCBVER, (void *)buffer);
			printf("Product PCB Version: %s\n",buffer);	
...
//#define BOOTLOADER_SW_VERSION  "sw_version"			
				...
				char sw_ver[64]=  {0};
				sprintf(header_buf, "%s", "sw_active");
				rtk_env_get(header_buf, str_active, sizeof(str_active));
				sscanf(str_active, "sw_active=%d", &swActiveA);
				//printf("swActiveA: %d\n", swActiveA);	
				sprintf(header_buf, "%s%d", "sw_version", swActiveA);
				rtk_env_get(header_buf, str_active, sizeof(str_active));
				sscanf(str_active, "%*[^=]=%s", sw_ver);			
				printf("Firmware Version: %s\n", sw_ver);
#if 0
				sprintf(header_buf, "%s%d", "sw_version", 1);
				rtk_env_get(header_buf, str_active, sizeof(str_active));
				sscanf(str_active, "%*[^=]=%s", sw_ver);			
				printf("Firmware Version 2: %s\n", sw_ver);
#endif
			}
#else
			printf("Firmware Version: %s\n", FIRMVERSION(FIRMVER));
