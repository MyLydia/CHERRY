...
int flash_read(void *buf, int offset, int len) /* raw flash read, without protection */
{
	...
	if (offset != g_cs_offset && offset != HW_SETTING_OFFSET && offset != DEFAULT_SETTING_OFFSET) {
		...
		if ((flash_idx = get_flash_index((void*)(intptr_t)offset)) < 0)
			return 0;
...
#ifdef CONFIG_USER_CONF_ON_XMLFILE
	return !_save_xml_file(data_type == HW_SETTING ? CONF_ON_XMLFILE_HS : CONF_ON_XMLFILE, data_type, 0);
#else
...
int _mib_load(CONFIG_DATA_T data_type) /* Load flash setting to RAM */
{
	...
	return !_load_xml_file(data_type == HW_SETTING ? CONF_ON_XMLFILE_HS : CONF_ON_XMLFILE, data_type, 0);
#else
...
#ifdef CONFIG_USER_CONF_ON_XMLFILE
	if (access(data_type == HW_SETTING ? CONF_ON_XMLFILE_DEFAULT_HS : CONF_ON_XMLFILE_DEFAULT, R_OK) == NULL)
...
#ifdef CONFIG_USER_CONF_ON_XMLFILE
	i = _save_xml_file(data_type == HW_SETTING ? CONF_ON_XMLFILE_HS : CONF_ON_XMLFILE, data_type, 0);
...
int mib_init(void) /* Initialize */
{
	...
	if(mib_init_mib_with_program_default(RUNNING_SETTING, FLASH_DEFAULT_TO_MEMORY))
		rs_valid = 1;
...
int _mib_cmp(int id, void *value) /* compare mib value */
{
	int i;
	int ret=0;
	unsigned char * pMibTbl;
...
//			TRACE("mib_get %s\n",mib_table[i].name);
	...
	case STRING_T:
		ret = strcmp((char *)value, (const char *)(pMibTbl + mib_table[i].offset));
		break;
	...
	#endif
	case BYTE_ARRAY_T:
		ret = memcmp(value, pMibTbl + mib_table[i].offset, mib_table[i].size);
		break;
