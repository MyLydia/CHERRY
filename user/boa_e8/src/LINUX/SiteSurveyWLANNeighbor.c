...
#define WIRELESS_NEIGHBOR_INFO_TBL_TMP	"/var/wireless_neighbor_info_temp"
...
int SiteSurveyWLANNeighbor(int wlan_if_index)
{
	static SS_STATUS_T Status={0};
	unsigned char table_name[36];
	...
	wlan_idx = wlan_if_index;
	snprintf(ifname, IFNAMSIZ, "%s", getWlanIfName());
	wlan_idx = orig_wlan_idx;
	if(check_wlan_net_device_trx_stats(ifname) == 1){
	...
	while (1) {
		Status.number=0;
		if ( getWlSiteSurveyResult(ifname, (SS_STATUS_Tp)&Status) < 0 ) {
	...
	if(!(fp_tmp = fopen(WIRELESS_NEIGHBOR_INFO_TBL_TMP, "w"))) {
		goto ss_err;
	}
	...
	{		
		/*printf("%-5d%-25s%02X:%02X:%02X:%02X:%02X:%02X %-7d%-5d%-5d%-5d\n"
			, bssdb_idx
			, Status.bssdb[bssdb_idx].bdSsIdBuf
			, Status.bssdb[bssdb_idx].bdBssId[0]
			, Status.bssdb[bssdb_idx].bdBssId[1]
			, Status.bssdb[bssdb_idx].bdBssId[2]
			, Status.bssdb[bssdb_idx].bdBssId[3]
			, Status.bssdb[bssdb_idx].bdBssId[4]
			, Status.bssdb[bssdb_idx].bdBssId[5]
			, Status.bssdb[bssdb_idx].bdType
			, Status.bssdb[bssdb_idx].ChannelNumber
			, Status.bssdb[bssdb_idx].network
			, Status.bssdb[bssdb_idx].rssi-100);*/
		fprintf(fp_tmp, "%-5d%-25s%02X:%02X:%02X:%02X:%02X:%02X %-7d%-5d%-5d%-5d\n"
			, bssdb_idx
	...
	fclose(fp_tmp);
	sprintf(table_name, "%s_wlan%d", WIRELESS_NEIGHBOR_INFO_TBL, wlan_if_index);
	unlink(table_name);
	rename(WIRELESS_NEIGHBOR_INFO_TBL_TMP, table_name);
	return 0;
ss_err:
	unlink(table_name);
	return -1;
...
int main(int argc, char *argv[])
{
	...
	{
		if(SiteSurveyWLANNeighbor(0) == -1)
			goto arg_err_rtn;
	}
	else if(!strcmp(argv[argIdx], "1"))
	{
		if(SiteSurveyWLANNeighbor(1) == -1)
			goto arg_err_rtn;
	}
	else
	{
...
{
	printf("Usage:\n");
	printf("	SiteSurveyWLANNeighbor 0\n");
	printf("	SiteSurveyWLANNeighbor 1\n");
}
