...
int igmproxyList(int eid, request * wp, int argc, char ** argv)
{
	...
	mib_get(MIB_IGMP_PROXY, &igmpProxy);
	_PUT_BOOL(igmpProxy);
...
int ifwanList(int eid, request * wp, int argc, char **argv)
{
	...
	for (i=0; i<entryNum; i++) {
		...
		if(wtype == 3)
		{
			if(!(((Entry.cmode == CHANNEL_MODE_IPOE||Entry.cmode == CHANNEL_MODE_PPPOA||\
			Entry.cmode == CHANNEL_MODE_PPPOE||Entry.cmode == CHANNEL_MODE_RT1483))\
			&&((Entry.applicationtype&X_CT_SRV_INTERNET)||(Entry.applicationtype&X_CT_SRV_OTHER))))
				continue;
		}
#endif
...
{
	char *strSnp, *submitUrl;
	char origmode = 0, mode;
	unsigned char snpEnable=1;
#if defined(CONFIG_RTL_IGMP_SNOOPING)
	strSnp = boaGetVar(wp, "snp", "");
	if(strSnp[0])
	{
		// bitmap for virtual lan port function
		// Port Mapping: bit-0
...
#endif
			printf("IGMP Snooping: enable, MLD Snooping: enable\n");
		}
		else
		{
			snpEnable=0;
...
#endif
			printf("IGMP Snooping: disable, MLD Snooping: disable\n");
		}
		mib_set(MIB_MPMODE, (void *)&mode);
		mib_set(MIB_IGMP_MLD_SNOOPING, (void *)&snpEnable);
	}
...
{
	char *submitUrl, *strProxy, *str_extif;
	MIB_CE_ATM_VC_T Entry;
	unsigned char proxyEnable=1, is_enabled;
	int entryNum, i;
	char tmpBuf[100];
	unsigned int ext_if;
	
	strProxy = boaGetVar(wp, "proxy", "");
#if defined(CONFIG_CMCC) || defined(CONFIG_CU)
	...
	if(strProxy[0])
	{
		...
		if(is_enabled)
		{
			if(!mib_set(MIB_MLD_PROXY_EXT_ITF, (void *)&ext_if))
			{
				...
				strcpy(tmpBuf, "UPNP Binded WAN interface index Éè¶¨´íÎó!");  //Set UPNP Binded WAN interface index error!
				goto setErr_igmp;
			}
		}
		startMLDproxy();
	...
	mib_set(MIB_IGMP_MLD_PROXY, &proxyEnable);
