...
int check_itfGroup(MIB_CE_ATM_VC_Tp pEntry, MIB_CE_ATM_VC_Tp pOldEntry)
{
	...
	if (pEntry->applicationtype&(X_CT_SRV_INTERNET | X_CT_SRV_SPECIAL_SERVICE_ALL)) {
		...
		//printf("\033[1;33;40m @@@@@@@@@@@@@@@@@########### %s:%d 1DO NOT rest port binding\033[m\n", __FUNCTION__, __LINE__);
		if(checkIPv4_IPv6_Dual_PolicyRoute_ex(pEntry, &wanIndex, &portMask)==1)
		{
			//printf("\033[1;33;40m @@@@@@@@@@@@@@@@@########### %s:%d 2DO NOT rest port binding\033[m\n", __FUNCTION__, __LINE__);
			return 0;
		}
#endif
		...
		for(i = 0; i < totalEntry; ++i)
		{
			if(mib_chain_get(MIB_ATM_VC_TBL, i, (void*)&temp_Entry))
			{
				...
				if(temp_Entry.applicationtype & (X_CT_SRV_INTERNET | X_CT_SRV_SPECIAL_SERVICE_ALL) && temp_Entry.vid == pEntry->vid)
					continue;
...
const char*  pol_mark[] = {"0x1000000/0xff000000", "0x2000000/0xff000000",
													...
													"0xf000000/0xff000000",
													"0x10000000/0xff000000",
													"0x20000000/0xff000000",
													"0x30000000/0xff000000",
													"0x40000000/0xff000000",
													"0x50000000/0xff000000",
#endif
													""};
...
static int disable_lan_dns(MIB_CE_ATM_VC_Tp pEntry)
{
	int32_t j;
	int32_t tbl_id;
	uint16_t itfGroup;
	...
	for(j = PMAP_ETH0_SW0; j <= PMAP_ETH0_SW3 && j < SW_LAN_PORT_NUM; ++j)
	{
		...
		{
			//AUG_PRT("%s-%d drop dns request lan=%d\n",__func__,__LINE__,j);
			// Drop dns requests from bounded LAN
			va_cmd(EBTABLES, 6, 1, FW_ADD, FW_DNS_DIS, "-i", SW_LAN_PORT_IF[j], "-j", FW_DROP);
		}
	}
#ifdef WLAN_SUPPORT
	for(j = PMAP_WLAN0; j <= PMAP_WLAN1_VAP3; ++j)
	{
		if(BIT_IS_SET(itfGroup, j))
		{
			if(wlan_en[j-PMAP_WLAN0] == 0)
				continue;
			// Drop dns requests from bounded WLAN
			va_cmd(EBTABLES, 6, 1, FW_ADD, FW_DNS_DIS, "-i", wlan[j - PMAP_WLAN0], "-j", FW_DROP);
...
static void disable_lan_dhcp(MIB_CE_ATM_VC_Tp pEntry)
{
	int32_t j;
	int32_t tbl_id;
	uint16_t itfGroup;
	...
	//AUG_PRT("%s-%d disableLanDhcp=%d\n",__func__,__LINE__,pEntry->disableLanDhcp);	
	itfGroup	= pEntry->itfGroup;
	//AUG_PRT("%s-%d itfGroup=%x\n",__func__,__LINE__,itfGroup);

	for(j = PMAP_ETH0_SW0; j <= PMAP_ETH0_SW3 && j < SW_LAN_PORT_NUM; ++j)
	{
		if(BIT_IS_SET(itfGroup, j))
		{
			//AUG_PRT("%s-%d disableLanDhcp=%d j=%d\n",__func__,__LINE__,pEntry->disableLanDhcp,j);
...
void add_prohibit_rule_for_speedtest(MIB_CE_ATM_VC_Tp pEntry)
{
	int j;
	
	for(j = PMAP_ETH0_SW0; j <= PMAP_ETH0_SW3 && j < SW_LAN_PORT_NUM; ++j)
	{
		if (pEntry->itfGroup & (1<<j))
		{
			va_cmd(BIN_IP, 5, 1, "rule", "add", "fwmark", pol_mark[j], "prohibit");
		}
	}
...
void del_prohibit_rule_for_speedtest(MIB_CE_ATM_VC_Tp pEntry)
{
	int j;
	...
	for(j = PMAP_ETH0_SW0; j <= PMAP_ETH0_SW3 && j < SW_LAN_PORT_NUM; ++j)
	{
		if (pEntry->itfGroup & (1<<j))
		{
			va_cmd(BIN_IP, 5, 1, "rule", "del", "fwmark", pol_mark[j], "prohibit");
		}
	}
#ifdef WLAN_SUPPORT
	for(j = PMAP_WLAN0; j <= PMAP_WLAN1_VAP_END; ++j)
...
static int getNetAddrwithMask(MIB_CE_ATM_VC_Tp pEntry, char* pAddr, int length)
{
	...
	intAddr = ntohl(in_net_addr.s_addr);
	inet_ntop(AF_INET, &intAddr, pAddr, INET_ADDRSTRLEN);
...
int pmap_reset_ip_route(MIB_CE_ATM_VC_Tp pEntry)
{
	...
	if (pEntry->cmode == CHANNEL_MODE_IPOE) {
		va_cmd(BIN_IP, 4, 1, "route", "flush", "table", str_tblid);
		if ((pEntry->IpProtocol == IPVER_IPV6)&& (pEntry->AddrMode == IPV6_WAN_DSLITE)) { 
			...
			intAddr = ntohl(*(unsigned int *)pEntry->remoteIpAddr);
...
//for iptv, if lan port binding to bridge other wan, we won't let igmp join 
//trap to PS, dut to if you have routing wan at the same time. it would forward it again.
static void stop_igmp_to_routingWan(MIB_CE_ATM_VC_Tp pEntry)
{
	int32_t j;
	int32_t tbl_id;
	uint16_t itfGroup;
	...
	for(j = PMAP_ETH0_SW0; j <= PMAP_ETH0_SW3 && j < SW_LAN_PORT_NUM; ++j)
	{
		if(BIT_IS_SET(itfGroup, j))
		{
			AUG_PRT("%s-%d stop_igmp_to_routingWan j=%d\n",__func__,__LINE__,j);
			// Drop igmp requests to routing Wan
			va_cmd(EBTABLES, 10, 1, FW_ADD, PORTMAP_IGMP, "-i", SW_LAN_PORT_IF[j], "-p", "IPv4", "--ip-proto", "2", "-j", FW_DROP);			
		}
	}
#ifdef WLAN_SUPPORT
	for(j = PMAP_WLAN0; j <= PMAP_WLAN1_VAP_END; ++j)
	{
		if(BIT_IS_SET(itfGroup, j))
		{
			if(wlan_en[j-PMAP_WLAN0] == 0)
				continue;
				// Drop DHCP requests from bounded LAN
				va_cmd(EBTABLES, 10, 1, FW_ADD, PORTMAP_IGMP, "-i", wlan[j - PMAP_WLAN0], "-p", "IPv4", "--ip-proto", "2", "-j", FW_DROP); 		
...
#if defined(CONFIG_LUNA_DUAL_LINUX) && defined(WLAN_DUALBAND_CONCURRENT)
int check_slave_wifi_bind_en()
{
#if defined(CONFIG_CMCC) || defined(CONFIG_CU)
	system("echo 1 > /proc/rg/slaveWifiBind");
	printf("dualband master cpu only mode\n");
#else
	int i, j;
	int totalEntry, itfGroup;
	char slave_wifi_en = 0;
	MIB_CE_ATM_VC_T temp_Entry;
	...
	for(i = 0; i < totalEntry; ++i){
		if(mib_chain_get(MIB_ATM_VC_TBL, i, (void*)&temp_Entry))
		{
			...
			for (j = PMAP_WLAN1; j <= PMAP_WLAN1_VAP_END; j++){
				if (BIT_IS_SET(temp_Entry.itfGroup, j)){
					slave_wifi_en = 1;
					break;
				}
			}
	...
	if (slave_wifi_en){
		...
		system("echo 0 > /proc/rg/slaveWifiBind");
		printf("dualband concurrent mode\n");
	}
#endif
	return 0;
}

int check_slave_wifi_en()
{
#if defined(CONFIG_CMCC) || defined(CONFIG_CU)
	system("echo 4 > /proc/vwlan");
	printf("dualband master cpu only mode\n");
#else
	int i, j;
	int totalEntry, itfGroup;
...
int exec_portmp()
{
	...
	for(i = 0; i < totalEntry; ++i)
	{
		if(mib_chain_get(MIB_ATM_VC_TBL, i, (void*)&temp_Entry))
		{
			if (!temp_Entry.enable)
				continue;
//AUG_PRT("%s-%d disableLanDhcp=%d, i=%d\n",__func__,__LINE__,temp_Entry.disableLanDhcp,i);
			if(temp_Entry.disableLanDhcp){
				disable_lan_dhcp(&temp_Entry);
			}
			disable_lan_dns(&temp_Entry);			
#ifdef CONFIG_USER_IGMPPROXY
			if(temp_Entry.cmode == CHANNEL_MODE_BRIDGE){
				stop_igmp_to_routingWan(&temp_Entry);
			}
#endif
			if((temp_Entry.cmode == CHANNEL_MODE_BRIDGE ||
...
static void reset_pmap()
{
	...
	va_cmd(EBTABLES, 2, 1, "-F", FW_DNS_DIS);	
}
...
#define LOCK_PMAP()	\
do {	\
	if ((lockfd = open(ConfigPmapLock, O_RDWR)) == -1) {	\
...
void setupnewEth2pvc()
{
	...
	int lan_binding_flag = 0;
	mib_get(PROVINCE_SICHUAN_MIB_LAN_BINDING_CONFIG_ENABLE, &lan_binding_flag);
	if(lan_binding_flag)
		RTK_RG_del_Lan_Binding_Acl();
#endif
	...
	RTK_RG_Sync_OMCI_WAN_INFO(SYNC_OMCI_WAN_INFO_FORWARD);
#endif
#endif
	//set_vc_fgroup();
...
#if (defined(CONFIG_CMCC) || defined(CONFIG_YUEME) || defined(CONFIG_CU)) && defined(CONFIG_RTK_L34_ENABLE)
...
int find_wanif_by_vlanid(unsigned short latvid, MIB_CE_ATM_VC_T* vc_Entry)
{
	...
	for(j = 0; j < totalEntry; ++j)
	{
		...
		if(vc_Entry->vlan && vc_Entry->vid == latvid)
		{
			...
			AUG_PRT("ifIndex=%x vc_Entry->cmode\n",ifIndex,vc_Entry->cmode);			
		}
	}
	return ifIndex;
...
void attach_mark_to_vlanpck()
{
	...
	check_cvlan_group_before_add_vlan_binding();
...
//--------------------------------------------------------------------------------------------
					...
					if(getATMVCEntryByIfIndex(ifindex, &vc_Entry)==NULL)
						continue;
#ifdef CONFIG_RTK_L34_ENABLE
					...
					if(!every_port_do_once)
					{
						...
						{
							char broute_chain_wlan_tmp[32]={0};
							snprintf(broute_chain_wlan_tmp, sizeof(broute_chain_wlan_tmp), "%s%s", BROUTE_CHAIN_PREFIX, wlan[port - PMAP_WLAN0]);
							//va_cmd(EBTABLES, 6, 1, "-t", "broute", "-A", (char *)VLANMAP_BROUTE, "-j", (char *)SW_LAN_PORT_IF[port - PMAP_WLAN0]);
							va_cmd(EBTABLES, 8,  1, "-t", "broute", "-I", VLANMAP_BROUTE, "-i", wlan[port - PMAP_WLAN0],
													"-j", broute_chain_wlan_tmp);
						}
#endif
					}
					...
					*/
					...
					if(CHANNEL_MODE_BRIDGE == vc_Entry.cmode)
					{
						...
						else
						{
							...
							char broute_chain_wlan_tmp[32]={0};
							snprintf(broute_chain_wlan_tmp, sizeof(broute_chain_wlan_tmp), "%s%s", BROUTE_CHAIN_PREFIX, wlan[port - PMAP_WLAN0]);
							va_cmd(EBTABLES, 14, 1, "-t", "broute", "-A", broute_chain_wlan_tmp,
													"-p", "8021q", "--vlan-id", str_vid_a,
													"-j", "mark", "--mark-or", str_mark, "--mark-target", "CONTINUE");
...
void reset_vlanmap()
{
	...
	for(j = PMAP_WLAN0; j <= PMAP_WLAN0_VAP_END; ++j)
	{
		...
		snprintf(broute_chain_wlan_tmp, sizeof(broute_chain_wlan_tmp), "%s%s", BROUTE_CHAIN_PREFIX, wlan[j - PMAP_WLAN0]);
		va_cmd(EBTABLES, 6, 1, "-t", "broute", "-N", (char *)broute_chain_wlan_tmp, "-P", "RETURN");
		//ebtables -F vlanmapping
		va_cmd(EBTABLES, 4, 1, "-t", "broute", "-F", (char *)broute_chain_wlan_tmp);
		//ebtables -I 1 FORWARD -j vlanmapping
		//va_cmd(EBTABLES, 6, 1, "-t", "broute", "-A", (char *)VLANMAP_BROUTE, "-j", (char *)wlan[j - PMAP_WLAN0]);
#ifdef CONFIG_RTK_L34_ENABLE
		RG_flush_vlanBinding(j);
		va_cmd(EBTABLES, 4, 1, "-t", "broute", "-F", (char *)VLAN_BIND, "-i", (char *)broute_chain_wlan_tmp);
#endif
	}
#endif
