...
int _wanConfList(int bShowIndex, int *map)
{
    ...
    for (i=0; i<entryNum; i++)
    {
        ...
        {
            strncpy(szUserName, Entry.pppUsername, MAX_PPP_NAME_LEN - 1);
            szUserName[MAX_PPP_NAME_LEN - 1] = '\0';
...
int atmConnList(int bShowIndex, int *map)
{
    ...
    for (i = 0; i < uiEntryNum; i++)
    {
        ...
        if (stEntry.enable == 0 || MEDIA_INDEX(stEntry.ifIndex) != MEDIA_ATM)
            continue;
