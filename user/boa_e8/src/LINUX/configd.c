...
int ctc_igd_shm_id; // shared memory ID
char *ctc_igd_shm_start; // attaching address
#endif
int this_pid;
...
void *ddnsC(struct ddns_info *tinfo)
{
	...
	for (i=0; i<entryNum; i++) {
		...
		for(j = 0 ; pEntry->provider[j] ; j++)
			provider[j] = tolower(pEntry->provider[j]);
		if ( strcmp(provider, "dyndns") == 0 || strcasecmp(provider, "tzo") == 0
			|| strcmp(provider, "oray") == 0) {
		...
		if (strcmp("Any", pEntry->interface) == 0) {
			if ( g_killprocess == KILL_PROCESS_OFF ) {
				...
				va_niced_cmd("/bin/updatedd", 3, 0, provider, account, pEntry->hostname);
			}
		}
		...
		// For IPv6
			...
			if (strncmp(tinfo->ifname, "ppp", 3) == 0) {
				...
				if ( num != 5) {
					inet_ntop(PF_INET6, &ip6_addr[0].addr, itfIPv6, INET6_ADDRSTRLEN);
					getV6IP = 1;
		...
		if ( g_killprocess == KILL_PROCESS_OFF ) {
			...
			if (getV4IP || getV6IP) {
				...
				syslog(LOG_INFO, "ddnsC: %s Start updatedd - provider is %s\n", tinfo->ifname, provider);
			}
			if ( (getV4IP && getV6IP) || getV4IP)
				va_niced_cmd("/bin/updatedd", 6, 0, provider, "--", "-4", itfIP, account, pEntry->hostname);
			...
			if (getV4IP && getV6IP)
				// updatedd dyndns -- -4 61.11.22.33 -6 2001:b010:7030:1a00:90:1a00:a2:8506 yufc:663333 mason2.dyndns.info
					va_niced_cmd("/bin/updatedd", 8, 0, provider, "--", "-4", itfIP, "-6", itfIPv6, account, pEntry->hostname);
			else if (getV4IP)
...
#ifdef CONFIG_USER_DBUS_CTC_IGD
static int initShm_for_ctc_igd()
{
	if ((ctc_igd_shm_id = shmget((key_t)CTC_IGD_SHM_KEY, SHM_SIZE*2, 0644 | IPC_CREAT)) == -1) {
		perror("shmget");
		return -1;
	}
	if ((ctc_igd_shm_start = (char *)shmat ( ctc_igd_shm_id , NULL , 0 ) )==(char *)(-1)) {
		perror("shmat");
		return -1;
	}
	//printf("initShm: shm_id=%d, shm_start=0x%x\n", shm_id, shm_start);
...
#include <sys/syscall.h>
extern inline int checkPidExist(int tid, int tgid);
...
#ifdef _SET_MSGQ_SIZE_AND_NONBLOCK_
				send_message_nonblock(qid_rcv, qbuf.mtype, qbuf.request, 0, &qbuf.msg);
#else
				send_message(qid_rcv, qbuf.mtype, qbuf.request, 0, &qbuf.msg);
#endif //_SET_MSGQ_SIZE_AND_NONBLOCK_
			}
		}
	}
...
int flashdrv_filewrite(FILE * fp, int old_size, void *dstP)
{
	...
	flash_pos_count = dstP - flash_mtd_info[flash_idx].start;
	nWritten = 0;
...
UPGRADE_IMGB:
	...
	if (backup) {
		...
		if ((flash_idx = get_flash_index(g_fs_bak_offset)) < 0) {
			ret = -1;
			goto fail;
		}
...
#ifdef CONFIG_USER_DBUS_CTC_IGD
	if (initShm_for_ctc_igd() < 0) {
		printf("Init shared memory for ctc-igd-server failed!\n");
		return -1;
	}
#endif
