	...
	for (i = 0; i < 4; i++)
	{
		if(pEntry->dhcpv6_opt16_enable[i])
		{
			...
			if(pEntry->dhcpv6_opt16_value_mode[i] == 0)
			{
				...
				(pEntry->dhcpv6_opt16_type[i] != 34 && pEntry->dhcpv6_opt16_type[i] != 32 && pEntry->dhcpv6_opt16_value_mode[i] == 1))
			{
				char buf[100] = {0};
				int ret;

				ret = gen_ctcom_dhcp_opt(pEntry->dhcpv6_opt16_type[i], buf, 100);
...
{
	unsigned char value[64], pidfile[30], leasefile[30], buf[128];
	...
	if (pEntry->IpProtocol & IPVER_IPV6) {
		...
		//Kernel 2.6.30 would not auto-generated IPv6 link-local Address
		...
		if ( pEntry->cmode == CHANNEL_MODE_IPOE || pEntry->cmode == CHANNEL_MODE_RT1483 || pEntry->cmode == CHANNEL_MODE_6RD ) {
			...
			if (pEntry->AddrMode & IPV6_WAN_STATIC) {
				...
				// Add WAN IP for MER
				snprintf(buf, sizeof(buf), "/%d", pEntry->Ipv6AddrPrefixLen); 
				strcat(Ipv6AddrStr, buf);
			...
			if ( (pEntry->AddrMode & 0x8) == 0x8 ) {
				...
				if(pEntry->SixrdIPv4MaskLen)
				{
					...
					snprintf((char*)buf2, sizeof(buf), "%s/%d", IPv4Mask,pEntry->SixrdIPv4MaskLen);
...
{
	char *argv[20];
	unsigned char value[32], buf[128], byte;
	int vInt, i, total;
	...
	for (i = 0; i < total; i++)
	{
		...
		// interface
		argv[idx++] = (char *)IP6TABLES;
		argv[idx++] = (char *)FW_ADD;
		if(IpEntry.dir == DIR_IN)
			argv[idx++] = (char *)FW_IPV6FILTER_IN;
		else
			argv[idx++] = (char *)FW_IPV6FILTER_OUT;
		...
		if(IpEntry.IpProtocol == IPVER_IPV6) //ipv6
			...
			AddRTK_RG_ACL_IPPort_Filter_IPv6(&IpEntry, in_action, out_action);
#endif
	}

	// Added by Mason Yu. for Telefonica ICMPV6 of firewall
	// ip6tables -A ipv6filter -p ICMPV6 -j RETURN
	va_cmd(IP6TABLES, 6, 1, (char *)FW_ADD, (char *)FW_IPV6FILTER_IN, "-p", (char *)ARG_ICMPV6, "-j", (char *)FW_RETURN);
	va_cmd(IP6TABLES, 6, 1, (char *)FW_ADD, (char *)FW_IPV6FILTER_OUT, "-p", (char *)ARG_ICMPV6, "-j", (char *)FW_RETURN);
...
int setup_default_IPV6Filter()
{
	...
	unsigned char value;
	unsigned char in_enable = 0, out_enable = 0;
	unsigned char action = 1;	//0: white list, 1: black list
...
#if defined(CONFIG_CMCC) || defined(CONFIG_CU)
	if ( !mib_get(MIB_DHCPV6S_DNS_ASSIGN_MODE, (void *)&ipv6PrefixMode)) {
	...
	{ 
		...
		case IPV6_DNS_WANCONN:
			...
			if(Entry.dnsv6Mode==REQUEST_DNS){//DNS from DHCPv6 server
				...
				inet_ntop(PF_INET6,Entry.Ipv6Dns1, tmpBuf, sizeof(tmpBuf));
				strcpy(dnsv6_1,tmpBuf);
				inet_ntop(PF_INET6,Entry.Ipv6Dns2, tmpBuf, sizeof(tmpBuf));
...
#if defined(CONFIG_CMCC) || defined(CONFIG_CU)
//Helper function for PrefixV6 mode of radvd
int get_prefixv6_info_radvd(PREFIX_V6_INFO_Tp prefixInfo)
{
	...
	switch(prefixInfo->mode){
		...
		case IPV6_PREFIX_STATIC:
			...
			if (!mib_get(MIB_V6_PREFIX_LEN, (void *)tmpBuf1)) {
...
//Helper function for PrefixV6 mode of dhcp
