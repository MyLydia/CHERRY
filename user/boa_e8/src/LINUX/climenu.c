	...
	while (1)
	{
		...
		switch(snum)
		{
			...
			case 3: // (3) Start PBC
				...
				mode = Entry.wlanMode;
				va_niced_cmd("/bin/wscd", 1, 1, "-sig_pbc");
...
int addEthWanConfiguration(MEDIA_TYPE_T m_type)
{
	...
	deleteConnection(CONFIGONE, NULL);
...
int setNewPortMapping(int mType)
{
	...
	char *map_ports[] = {"LAN_1", "LAN_2", "LAN_3", "LAN_4", "WLAN0", "WLAN0-AP1", "WLAN0-AP2", "WLAN0-AP3", "WLAN0-AP4"
														   ...
														   , "WLAN0-AP5", "WLAN0-AP6", "WLAN0-AP7"
#endif
														   , "WLAN1", "WLAN1-AP1", "WLAN1-AP2", "WLAN1-AP3", "WLAN1-AP4"
#ifdef WLAN_8_SSID_SUPPORT
														   , "WLAN1-AP5", "WLAN1-AP6", "WLAN1-AP7" 
#endif
														   };
...
static void updateImage_FTP() {
	...
	if (cmd_check_image(fname, 0)) {
		...
		fclose(fp2);
		cmd_upload(fname, 0, st.st_size, 1);
#ifndef CONFIG_LUNA_FIRMWARE_UPGRADE_SUPPORT
	}
#endif
}
	...
	if ((fp = fopen("/etc/TZ", "w")) != NULL) {
		...
		fclose(fp);
				...
				if (dst_enabled || !is_tz_dst_exist(sel_list)) {
					snprintf(tz_location,sizeof(tz_location),"%s",get_tz_location_cli(sel_list));
...
int setSNTPServer()
{
	...
	char strbuf[MAX_NAME_LEN], *ptr;

	min = 1;
	max = 8;
	printf("Time Zone List for the first SNTP Server:\n");
	printf("\t(1) clock.fmt.he.net\n");
	printf("\t(2) clock.nyc.he.net\n");
	printf("\t(3) clock.sjc.he.net\n");
	printf("\t(4) clock.via.net\n");
	printf("\t(5) ntp1.tummy.com\n");
	printf("\t(6) time.cachenetworks.com\n");
	printf("\t(7) time.nist.gov\n");
	printf("\t(8) Other\n");
	...
	if (sel_list == 8) {
		...
		ptr = NTPServer[sel_list];
	}

	if (mib_set(MIB_NTP_SERVER_HOST1, ptr) == 0) {
		printf("Set the first SNTP Server from List error!");
		return 0;
	}

	min = 0;
	max = 8;
	printf("Time Zone List for the second SNTP Server:\n");
	printf("\t(0) None\n");
	printf("\t(1) clock.fmt.he.net\n");
	printf("\t(2) clock.nyc.he.net\n");
	printf("\t(3) clock.sjc.he.net\n");
	...
	if (mib_set(MIB_NTP_SERVER_HOST2, ptr) == 0) {
		printf("Set the second SNTP Server from List error!");
		return 0;
	}
...
#ifdef CTC_TELNET_LOGIN_WITH_NORMAL_USER_PRIVILEGE
#define LOGIN_TRY_MAX CTC_TELNET_LOGIN_TRY_MAX
int login_admin()
{
	int login_num = 0;

	char suusr[MAX_NAME_LEN];
	char supass[MAX_NAME_LEN];
	char user[MAX_NAME_LEN], passwd[MAX_NAME_LEN];

#if 0
	mib_get(MIB_SUSER_NAME, (void*)suusr);
	mib_get(MIB_SUSER_PASSWORD, (void*)supass);
#else
	// use HW setting first
	...
	while (1)
	{
		...
		printf("Please input User Name:");
		if (getInputStr(user, sizeof(user) - 1, NULL) == 0) return 0;
		if (user[strlen(user) - 1] == '\n')
			user[strlen(user) - 1] = 0;

		printf("Please input Password:");
		if (getInputStr(passwd, sizeof(passwd) - 1, NULL) == 0) return 0;
		if (passwd[strlen(passwd) - 1] == '\n')
			passwd[strlen(passwd) - 1] = 0;
	
		if(!strcmp(user, suusr) && !strcmp(passwd, supass))
		{
			return 1;
		}else{
			if((++login_num) < LOGIN_TRY_MAX) {
...
int setAdminLogin(SECURITY_LEVEL level)
{
	if (loginLevel > level) {
		int snum;
	...
	while (1)
	{
		...
		else
		{
			...
			switch( snum)
			{
				case 0:// shell
#ifdef CTC_TELNET_LOGIN_WITH_NORMAL_USER_PRIVILEGE
					if(!setAdminLogin(SECURITY_SUPERUSER))
						break;
#else
					if (!check_access(SECURITY_SUPERUSER))
						break;
