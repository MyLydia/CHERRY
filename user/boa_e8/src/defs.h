...
#define   strKeySelectedMustExist		"Key index selected必须存在!" //Key selected must exist!
#define   strInvdKeySel				"不合法的key index selected值!" //Invalid key index selected value
#endif
...
#if defined(CONFIG_CMCC) || defined(CONFIG_CU)
#define strSetPassAuthEnableerror "设定CPE password auth启用失败!" //Set GUI Password Auth Enable error!
#endif
