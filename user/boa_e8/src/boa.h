...
#define escape_string(a,z) _escape_string(a, z, sizeof(z)-1)
...
extern int free_from_login_list(request * req);
#ifdef ONE_USER_LIMITED
extern int free_from_login_list_by_ip_addr(char * remote_ip_addr);
#endif
...
#if defined(CONFIG_CMCC) || defined(CONFIG_CU)
#define OMCI_COMPLETE_CONFIG 1

typedef struct omci_msg2boaData_s {
	long mtype;
	char msgqData[32]; 
} omci_msg2boaData_t;
