...
#if defined(CONFIG_GENERAL_WEB)
		&& strstr(req->request_uri,"adminMenu.js") == NULL 
		&& strstr(req->request_uri,"userMenu.js") == NULL 
#endif
		) {   
...
//	req->last_modified = statbuf.st_mtime;
	...
	char *data_mem = req->data_mem;
	unsigned long filesize = req->filesize;
	unsigned long filepos = req->filepos;

	send_r_request_ok(req);		/* All's well */

#if CONFIG_USER_BOA_CSRF
	int orgfilestart = req->buffer_end;
#endif
#if 0
	{
		int bob;
...
#else
	{
		...
		unsigned char *left,*right,*last_right=data_mem;
		int bob;
		...
		while(1)
		{
			...
			if((left!=NULL) && (right!=NULL))
			{
				...
				if(bob>=0)
				{
					memcpy(req->buffer + req->buffer_end,	data_mem + filepos, bob);
					last_right=right+2;
					req->buffer_end += bob;
					filepos += (bob+(unsigned int)last_right-(unsigned int)left);
			...
			else
			{
				...
				bob=(unsigned int)data_mem+filesize-(unsigned int)last_right;
				while((bob+req->buffer_end)>req->max_buffer_size)
				{
					int ret;
					ret=allocNewBuffer(req);
...
//csrf token
// We duplicate the html file for add csrf token, the steps are as following
// 1. create the new html file
// 2. add csrf token before "</FORM>" or "</form>" token
// 3. mmap the html file to virtual memory
// 4. replace the virtual memory by using new one
// 5. unmap the new one
// 6. remove the new html file
#ifdef CONFIG_USER_BOA_CSRF
	//printf("%s:%d real_path_name %s\n", __FUNCTION__, __LINE__, real_path_name);
	//printf("%s:%d req->request_uri %s\n", __FUNCTION__, __LINE__, req->request_uri);
	//printf("%s:%d req->referer %s\n", __FUNCTION__, __LINE__, req->referer);
	char *orgdata_mem=&req->buffer[orgfilestart];
	int orgfilesize=req->buffer_end - orgfilestart;		/* filesize */
	char* path = req->pathname;
	char* path_slash=NULL;
	char* path_last_slash=NULL;
	char path_dir[128]={0};
	char real_path_dir[256]={0};
	char real_path_name[256]={0};
	char cmd[512]={0};
	FILE *fp;
	
	path_slash = strstr(path, "/");
	path_last_slash = path_slash;
	while(path_slash)
	{
		path = path_slash+1;
		path_slash = strstr(path, "/");
	...
	memcpy(path_dir, req->pathname, path_last_slash-req->pathname);
	//printf("path_dir %s\n",path_dir );
	
	//create dir
	sprintf(real_path_dir, "/tmp/%s", path_dir);
	sprintf(real_path_name, "/tmp/%s", req->pathname);
	sprintf(cmd, "mkdir -p %s", real_path_dir);
	system(cmd);
	...
	fwrite(&req->buffer[orgfilestart], 1, orgfilesize, fp);
		
	fclose(fp);

	//mmap to virtual memory
	char *csrf_data_mem;
	unsigned long csrf_filesize;		/* filesize */
	...
	/* MAP_OPTIONS: see compat.h */
	csrf_data_mem = mmap(0, csrf_filesize+10,
#ifdef USE_NLS
			PROT_READ|PROT_WRITE
#else
			PROT_READ
	...
	req->buffer_end = orgfilestart;

	data_mem = req->data_mem;
	filesize = req->filesize;
	filepos = 0;

	{
		//parse and send asp page
		unsigned char *left,*last_right=data_mem;
		int bob;
		char csrf_buf[128]={0};

		//generate csrf token string
		time_counter = getSYSInfoTimer();
		sprintf(csrf_buf, "%d+%s", time_counter, req->referer);

		// generate csrf token, exclude share.js
		if(strstr(req->request_uri,"share.js")==NULL){
			get_csrf_hash(csrf_buf, (char *)g_csrf_token);
		}

		sprintf(csrf_buf, "<input type='hidden' name='%s' value='%s' />\r\n", CSRF_TOKEN_STRING, g_csrf_token);
		while(1)
		{
			left = strstr(last_right,"</FORM>");
			if(left==NULL){
				left = strstr(last_right,"</form>");
			}
			...
			if(left!=NULL){
				...
				while((bob+ strlen(csrf_buf) + 7 +req->buffer_end)>(req->max_buffer_size))
				{
					int ret;
					ret=allocNewBuffer(req);
					if(ret==FAILED) {bob=req->max_buffer_size- req->buffer_end; break;}
				...
				if(bob > 0)
				{
					...
					// copy CSRF content
					memcpy(req->buffer + req->buffer_end, csrf_buf, strlen(csrf_buf));
					req->buffer_end += strlen(csrf_buf);

					// copy </FORM>
					memcpy(req->buffer + req->buffer_end,	data_mem + filepos, strlen("</FORM>"));
					req->buffer_end += strlen("</FORM>");
					filepos += strlen("</FORM>");

					last_right = left + strlen("</FORM>");
				}
			}
			else
			{
				bob=filesize - filepos;
				while((bob+req->buffer_end)>req->max_buffer_size)
				{
					int ret;
					ret=allocNewBuffer(req);
	...
	sprintf(cmd, "rm -rf %s", real_path_dir);
	system(cmd);
#endif
