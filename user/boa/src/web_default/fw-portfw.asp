...
<body>
<blockquote>
<h2 class="page_title"><% multilang(LANG_PORT_FORWARDING); %></h2>
...
<tr><th><% multilang(LANG_PORT_FORWARDING); %>:
	<input type="radio" value="0" name="portFwcap" <% checkWrite("portFw-cap0"); %> onClick="updateState()"><% multilang(LANG_DISABLE); %>&nbsp;&nbsp;
	<input type="radio" value="1" name="portFwcap" <% checkWrite("portFw-cap1"); %> onClick="updateState()"><% multilang(LANG_ENABLE); %>&nbsp;&nbsp;</th>
	<td><input type="submit" value="<% multilang(LANG_APPLY_CHANGES); %>" name="apply" onClick="return on_submit(this)">&nbsp;&nbsp;
</td></tr>
<tr><td><hr size=1 noshade align=top></td></tr>
</table>
<table>
...
<table>
    <tr>
		...
		<th>
			<% multilang(LANG_LOCAL_IP); %>
		</th>
		<th>
			<% multilang(LANG_LOCAL_PORT_FROM); %>
		</th>
		<th>
			<% multilang(LANG_LOCAL_PORT_TO); %>
		</th>
		<th>
			<% multilang(LANG_PROTOCOL); %>
		</th>
 	<th <% checkWrite("rg_hidden_function"); %>>
			<% multilang(LANG_REMOTE_IP); %>
		</th>
 	<th>
			<% multilang(LANG_REMOTE_PORT_FROM); %>
		</th>
 	<th>
			<% multilang(LANG_REMOTE_PORT_TO); %>
		</th>
 	<th>
			<% multilang(LANG_INTERFACE); %> <br>
		</th>
    </tr>
	...
	<tr>
		<td>
			<input type="submit" value="<% multilang(LANG_ADD); %>" name="addPortFw" onClick="return addClick(this)">
...
<form action=/boaform/formPortFw method=POST name="formPortFwDel">
	...
	<br><input type="submit" value="<% multilang(LANG_DELETE_SELECTED); %>" name="deleteSelPortFw" onClick="return deleteClick(this)">&nbsp;&nbsp;
	    <input type="submit" value="<% multilang(LANG_DELETE_ALL); %>" name="deleteAllPortFw" onClick="return deleteAllClick(this)">&nbsp;&nbsp;&nbsp;
