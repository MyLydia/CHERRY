...
function showDhcpSvr(ip)
{
	...
	if (document.dhcpd.dhcpdenable[0].checked == true)
		document.getElementById('displayDhcpSvr').innerHTML=
			'<input type="submit" value="<% multilang(LANG_APPLY_CHANGES); %>" name="save" onClick="return saveClick(0)">&nbsp;&nbsp;';
		...
		document.getElementById('displayDhcpSvr').innerHTML=
			...
			'</tr>'+
			'</table>'+
			'<input type="submit" value="<% multilang(LANG_APPLY_CHANGES); %>" name="save" onClick="return saveClick(1)">&nbsp;&nbsp;';
