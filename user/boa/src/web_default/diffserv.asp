...
<title><% multilang(LANG_DIFFERENTIATED_SERVICES); %></title>
...
<body>
<blockquote>
<h2 class="page_title"><% multilang(LANG_DIFFERENTIATED_SERVICES); %></h2>

<table>
<tr><td colspan=4><font size=2>
 <% multilang(LANG_ENTRIES_IN_THIS_TABLE_ARE_USED_TO_ASSIGN_THE_PRECEDENCE_FOR_EACH_INCOMING_PACKET_BASED_ON_TCP_UDP_PORT_NUMBER_SOURCE_DESTINATION_IP_ADDRESS_SUBNET_MASKS); %>
</font></td></tr>
...
<tr>
	<th><% multilang(LANG_DIFFERENTIATED_SERVICES); %>:</th>
	<td><font size=2>
		<input type="radio" name=qoscap value=0 onClick="return adminClick()"><% multilang(LANG_DISABLED); %>&nbsp;&nbsp;
		<input type="radio" name=qoscap value=1 onClick="return adminClick()"><% multilang(LANG_ENABLED); %>
	</td>
</tr>
<TBODY id='diffservSetting'>
<tr>
	<th><% multilang(LANG_PHB_GROUPS); %>:</td>
...
<tr>
	...
	<td></td>
	<th><% multilang(LANG_MIN); %>: 100Kb; <% multilang(LANG_MAX); %>: <% multilang(LANG_TOTAL_BANDWIDTH_LIMIT); %> - 100Kb</th>
</tr>
<tr>
...
<tr>
	<th><% multilang(LANG_MAX_DESIRED_LATENCY); %>:</th>
...
<p><input type="submit" value="<% multilang(LANG_APPLY_CHANGES); %>" name=addqos onClick="return addClick(this.form.phb)">
