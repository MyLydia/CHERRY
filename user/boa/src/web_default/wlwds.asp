...
<title>WDS <% multilang(LANG_SETTINGS); %></title>
...
//	return true;
	...
	if (checkString(document.formWlWdsAdd.comment.value) == 0) {
		alert("<% multilang(LANG_INVALID_COMMENT); %>");
...
<body>
<blockquote>
<h2 class="page_title">WDS <% multilang(LANG_SETTINGS); %></h2>
<form action=/boaform/admin/formWlWds method=POST name="formWlWdsAdd">
<table>
<tr><td><font size=2>
 <% multilang(LANG_WIRELESS_DISTRIBUTION_SYSTEM_USES_WIRELESS_MEDIA_TO_COMMUNICATE_WITH_OTHER_APS_LIKE_THE_ETHERNET_DOES_TO_DO_THIS_YOU_MUST_SET_THESE_APS_IN_THE_SAME_CHANNEL_AND_SET_MAC_ADDRESS_OF_OTHER_APS_WHICH_YOU_WANT_TO_COMMUNICATE_WITH_IN_THE_TABLE_AND_THEN_ENABLE_THE_WDS); %>
</font></td></tr>
</table>
...
<table width="800">
	...
	<tr>
	   <th>
	   <input type="checkbox" name="wlanWdsEnabled" value="ON" <% checkWrite("wlanWdsEnabled"); %>  onclick="updateState()">&nbsp;&nbsp;<% multilang(LANG_ENABLE); %> WDS</th>
	   <td><input type="submit" value="<% multilang(LANG_APPLY_CHANGES); %>" name="wdsSet" onClick="return on_submit(this)">&nbsp;&nbsp;</td>
	</tr>
</table>
...
<% checkWrite("wl_txRate"); %>
	...
	rate_name = ["<% multilang(LANG_AUTO); %>","1M","2M","5.5M","11M","6M","9M","12M","18M","24M","36M","48M","54M", "MCS0", "MCS1" , "MCS2", "MCS3", "MCS4", "MCS5", "MCS6", 
	...
	<tr <% checkWrite("wlan_qtn_hidden_function"); %>>
		<th nowrap><% multilang(LANG_COMMENT); %>:</th>
	...
	<tr>
		<td colspan=2>
			<input type="submit" value="<% multilang(LANG_ADD); %>" name="addWdsMac" onClick="return addClick(this)">&nbsp;&nbsp;
			<input type="reset" value="<% multilang(LANG_RESET); %>" name="reset">&nbsp;&nbsp;&nbsp;&nbsp;
			<input type="button" value="<% multilang(LANG_SET_SECURITY); %>" name="showWep" onClick="showWepClick('/wlwdsenp.asp')">&nbsp;&nbsp;
			<input type="button" value="<% multilang(LANG_SHOW_STATISTICS); %>" name="showAp" onClick="showApClick('/wlwdstbl.asp')">&nbsp;&nbsp;
...
<form action=/boaform/admin/formWlWds method=POST name="formWlWdsDel">
  <table>
  <tr><font size=2><b><% multilang(LANG_CURRENT_WDS_AP_LIST); %>:</b></font></tr>
  <% wlWdsList(); %>
  </table>
  <br>
  <input type="submit" value="<% multilang(LANG_DELETE_SELECTED); %>" name="deleteSelWdsMac" onClick="return deleteClick(this)">&nbsp;&nbsp;
  <input type="submit" value="<% multilang(LANG_DELETE_ALL); %>" name="deleteAllWdsMac" onClick="return deleteAllClick(this)">&nbsp;&nbsp;&nbsp;
  <input type="reset" value="<% multilang(LANG_RESET); %>" name="reset">
  <input type="hidden" value="/admin/wlwds.asp" name="submit-url">
  <input type="hidden" name="wlan_idx" value=<% checkWrite("wlan_idx"); %>>
  <input type="hidden" name="postSecurityFlag" value="">
