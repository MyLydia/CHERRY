...
<BODY>
<blockquote>
<h2 class="page_title"><% multilang(LANG_DSL_WAN); %><% multilang(LANG_CONFIGURATION); %></h2>
...
<table>
	<tr>
		<th>
			...
			VCI: 
			<input type="text" name="vci" size="6" maxlength="5"></td>
		<th><% multilang(LANG_ENCAPSULATION); %>: 
	...
	<tr>
		<th>
			<% multilang(LANG_ADMIN_STATUS); %>:</th>
...
<table>
	<td class="table_item">Enable VLAN:
...
<table>
<td class="table_item"><% multilang(LANG_DEFAULT_ROUTE); %>:
...
<input type="submit" value="<% multilang(LANG_MODIFY); %>" name="modify" onClick="return vcCheck(this)">
<BR>
<BR>
...
<br>
<input type="submit" value="<% multilang(LANG_DELETE_SELECTED); %>" name="delvc" onClick="return deleteClick(this)">
