...
<title><% multilang(LANG_DEVICE); %> <% multilang(LANG_IP_RANGE); %><% multilang(LANG_TABLE_2); %></title>
...
function addClick(obj) 
{
	...
	if (document.ipRange.Opt60.value=="") {		
		alert('<% multilang(LANG_OPTION60_CANNOT_BE_NULL); %>');
		document.ipRange.Opt60.focus();
		return false;
	}

	if ( !checkInputIP(document.ipRange.sIp.value ) ) {		
		alert('<% multilang(LANG_INVALID_SOURCE_RANGE_OF_IP_ADDRESS_IT_SHOULD_BE_IN_IP_POOL_RANGE); %>');
		document.ipRange.sIp.focus();
		return false;
	}

	if ( !checkInputIP(document.ipRange.dIp.value ) ) {		
		alert('<% multilang(LANG_INVALID_DESTINATION_RANGE_OF_IP_ADDRESS_IT_SHOULD_BE_IN_IP_POOL_RANGE); %>');
		document.ipRange.dIp.focus();
		return false;
	}
	...
	if(document.ipRange.dhcpDeviceType.selectedIndex==3) {
		if (document.ipRange.stbdns1.value == "") {			
			alert('<% multilang(LANG_ENTER_STBS_DNS_VALUE); %>');
		...
		if (document.ipRange.opchaddr.value == "") {			
			alert('<% multilang(LANG_ENTER_STBS_OPCH_ADDRESS_VALUE); %>');
		...
		if (document.ipRange.opchport.value == "") {			
			alert('<% multilang(LANG_ENTER_STBS_OPCH_PORT_VALUE); %>');
			document.ipRange.opchport.focus();
			return false;
		}
		if ( !checkDigitRange(document.ipRange.opchport.value,1,1,65535) ) {		  	
			alert('<% multilang(LANG_INVALID_OPCH_PORT_IT_SHOULD_BE_1_65535); %>');
			document.ipRange.opchport.focus();
			return false;
		}
	}
	else if(document.ipRange.dhcpDeviceType.selectedIndex!=5)
	{
		if(document.ipRange.OptStr.value == "") {			
			alert('<% multilang(LANG_ENTER_OPTION_STRING_VALUE); %>');
			document.ipRange.OptStr.focus();
			return false;
		}
	}
...
<body>
<blockquote>
<h2 class="page_title"><% multilang(LANG_DEVICE); %> <% multilang(LANG_IP_RANGE); %><% multilang(LANG_TABLE_2); %></h2>

<table>
  <tr><td><font size=2>
 <% multilang(LANG_THIS_PAGE_IS_USED_TO_CONFIGURE_THE_IP_ADDRESS_RANGE_BASED_ON_DEVICE_TYPE); %>
  </font></td></tr>
...
<table class="flat">
<tr>
    <th><% multilang(LANG_DEVICE); %><% multilang(LANG_NAME); %>:&nbsp;&nbsp;</th>
    <td><input type="text" name="name" size="20" maxlength="20"></td>
</tr>
    <th><% multilang(LANG_START_ADDRESS); %>:&nbsp;&nbsp;</th>
    <td><input type="text" name="sIp" size="20" maxlength="15"></td>
</tr>
<tr>
    <th><% multilang(LANG_END_ADDRESS); %>:&nbsp;&nbsp;</th>
    <td><input type="text" name="dIp" size="20" maxlength="15"></td>
</tr>
<tr>
    <th><% multilang(LANG_GATEWAY_ADDRESS); %>:&nbsp;&nbsp;</th>
    <td><input type="text" name="gwIp" size="20" maxlength="15"></td>
</tr>
<tr>
    <th><% multilang(LANG_OPTION60); %>:&nbsp;&nbsp;</th>
...
<tr>
	<th><% multilang(LANG_DEVICE_TYPE); %>:&nbsp;&nbsp;</th>
	<td><select size="1" name="dhcpDeviceType" onChange="deviceTypeSelect()">
		<option selected value=0><% multilang(LANG_PC); %></option>
		<option value=1><% multilang(LANG_CAMERA); %></option>
		<option value=2><% multilang(LANG_HGW); %></option>
		<option value=3><% multilang(LANG_STB); %></option>
		<option value=4><% multilang(LANG_PHONE); %></option>
		<option value=100><% multilang(LANG_UNKNOWN); %></option></select>
	</td>
</tr>
</table>
...
<table>
	 <tr>
		<th><% multilang(LANG_RESERVED_OPTION); %>:&nbsp;&nbsp;</th>
	 ...
	 <tr>
		<th><% multilang(LANG_OPTION_STRING); %></th>
...
<table>	
	<tr>
		<th><% multilang(LANG_RESERVED_OPTION); %>:&nbsp;&nbsp;</th>
		<td><input type="text" name="reservedOption240"  size="20" maxlength="20" value="Option 240" readonly></th>
	 </tr>
	 <tr>
		<th><% multilang(LANG_OPTION_STRING); %>:&nbsp;&nbsp;</th>
	 ...
	 <tr>
	 	<th><% multilang(LANG_DNS1); %>:</th>
	 	<td><input type="text" name="stbdns1" value=<% getInfo("stb-dns1"); %>></th>
	 </tr>
	 <tr>
	 	<th><% multilang(LANG_DNS2); %>:</th>
	 	<td><input type="text" name="stbdns2" value=<% getInfo("stb-dns2"); %>></td>
	 </tr>
</table>
</div>

<input type="submit" value="<% multilang(LANG_ADD); %>" name="add" onClick="return addClick(this)">&nbsp;&nbsp;
<input type="submit" value="<% multilang(LANG_DELETE); %>" name="del" onClick="return delClick()">&nbsp;&nbsp;
<input type="submit" value="<% multilang(LANG_MODIFY); %>" name="mod" onClick="return addClick(this)">&nbsp;&nbsp
...
<table>
<tr><font size=2><b><% multilang(LANG_DEVICE); %> <% multilang(LANG_IP_RANGE); %><% multilang(LANG_TABLE_2); %>:</b></font></tr>
    <% showDeviceIpTable(); %>
</table>
