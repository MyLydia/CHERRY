...
<title><% multilang(LANG_SYSTEM_DATE_AND_TIME_MODIFICATION); %></title>
...
<body>
<blockquote>
<h2 class="page_title"><% multilang(LANG_SYSTEM_DATE_AND_TIME_MODIFICATION); %></h2>

<table>
  <tr><td><font size=2>
    <% multilang(LANG_SET_THE_SYSTEM_DATE_AND_TIME); %>
  </font></td></tr>
...
<table>
  ...
  <tr>
      <th><% multilang(LANG_TIME_ZONE); %>:</th>
      <td width=70%>
      	<select name="sys_tz">
...
<input type="hidden" value="/admin/date.asp" name="submit-url">
  <p><input type="submit" value="<% multilang(LANG_SUBMIT); %>" name="set" onClick="return on_submit()">&nbsp;&nbsp;
  <input type="button" value="<% multilang(LANG_CANCEL); %>" name="close" onClick="javascript: window.close();"></p>
