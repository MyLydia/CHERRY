...
void make_dhcpcv6_conf(MIB_CE_ATM_VC_Tp pEntry, char *dhcpc_cf_buf, int size)
{
	...
	snprintf(dhcpc_cf_buf,size,"/var/%s%s.conf", DHCPCV6STR, wanif);
	...
	if((pEntry->dslite_enable == 1) && (pEntry->dslite_aftr_mode == IPV6_DSLITE_MODE_AUTO)){
		fprintf(conf_fp, "option dhcp6.dslite code 64 = domain-list;\n");
		fprintf(conf_fp, "also request dhcp6.dslite;\n");
	}
#endif
	#if 0
	// DUID based on Link-layer Address [DUID-LL]; if not specified, default is DUID-LLT.
	ifGetName(PHY_INTF(pEntry->ifIndex), wandev, sizeof(wandev));
	getInAddr(wandev, HW_ADDR, &hwaddr);
	fprintf(conf_fp, "send dhcp6.client-id 00:03:00:01:%02X:%02X:%02X:%02X:%02X:%02X;\n",
	...
	#if 0
	if ((pEntry->Ipv6DhcpRequest & 0x04) == 0x04) {
		fprintf(conf_fp, "send dhcp6.rapid-commit;\n");
	}
	#endif
	#endif
