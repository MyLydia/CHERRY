...
{	
	...
	if ( myip == NULL && fh != 0)	// It is for DHCP dynamic connection
	{
		...
		if (extPort) {			
			snprintf(buff, sizeof(buff), "\tiptables -t nat -A %s -s %s -d $ip -p %s --dport %s -j DNAT --to-destination %s\n", nat_pre_name, lan_ipaddr, (char *)proto, extPort, dstIP);	
			WRITE_DHCPC_FILE(fh, buff);
		} else {			
			snprintf(buff, sizeof(buff), "\tiptables -t nat -A %s -s %s -d $ip -p %s -j DNAT --to-destination %s\n", nat_pre_name, lan_ipaddr, (char *)proto, dstIP);	
			WRITE_DHCPC_FILE(fh, buff);
		}
