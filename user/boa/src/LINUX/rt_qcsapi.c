...
static void qtn_band(int band, char *mode, unsigned int len)
{
	switch(band){
		case BAND_11B:
			snprintf(mode, len, "11b");
			break;
		case BAND_11G:
			snprintf(mode, len, "11g");
			break;
		case BAND_11A:
			snprintf(mode, len, "11a");
			break;
		//case BAND_11N:
		//	snprintf(mode, sizeof(mode), "11n");
		//	break;
		case (BAND_11G|BAND_11N):
			snprintf(mode, len, "11ng");
			break;
		case BAND_5G_11AN:
			snprintf(mode, len, "11na");
			break;
		case BAND_5G_11AC:
			snprintf(mode, len, "11ac");
			break;
	}
}
...
int setupWLan_qtn(char *ifname, int index)
{
	...
	if(index==0){
		...
		qtn_band(vChar, parm, sizeof(parm));
...
int rt_qcsapi_get_properties_AP(char *ifname, SS_STATUS_Tp pStatus)
{
	...
	for(i=0; i<count; i++){
		...
		memcpy(pStatus->bssdb[i].bssid, ap_properties.ap_mac_addr, MAC_ADDR_SIZE);
		memcpy(pStatus->bssdb[i].ssid, ap_properties.ap_name_SSID, QCSAPI_SSID_MAXLEN); 
		pStatus->bssdb[i].ssidlen = strlen(ap_properties.ap_name_SSID);
		...
		if(ap_properties.ap_flags & qcsapi_ap_security_enabled){
			...
			if(ap_properties.ap_encryption_modes & qcsapi_ap_TKIP_encryption_mask){//TKIP
				...
				if(ap_properties.ap_protocol & qcsapi_protocol_11i_mask) // WPA2
					pStatus->bssdb[i].t_stamp[0] |= (0x1 << 24);
				
			}
			if(ap_properties.ap_encryption_modes & qcsapi_ap_CCMP_encryption_mask){//CCMP
				if(ap_properties.ap_authentication_mode == qcsapi_ap_PSK_authentication) //PSK
					pStatus->bssdb[i].t_stamp[0] |= (0x4 << 28);
				else //EAP
					pStatus->bssdb[i].t_stamp[0] |= (0x2 << 28);
					
				if(ap_properties.ap_protocol & qcsapi_protocol_WPA_mask) // WPA
					pStatus->bssdb[i].t_stamp[0] |= (0x4 << 8);
				if(ap_properties.ap_protocol & qcsapi_protocol_11i_mask) // WPA2
					pStatus->bssdb[i].t_stamp[0] |= (0x4 << 24);
			}
		}	
	}
