...
#ifdef CONFIG_DEV_xDSL
void showDSLSettings(XDSL_OP *d)
{
	unsigned char vChar;
	...
	if (!dsl_anxb) {
		...
		printf("AnnexJ Option:\n");	// AnnexJ Option
		if (mode & ADSL_MODE_ANXJ)
			printf("\tAnnexJ:\t\t  Enable\n");
		else
			printf("\tAnnexJ:\t\t  Disable\n");
	}
...
static void _DSLuptime(void)
{
	XDSL_OP * d;
	Modem_LinkSpeed vLs;
	unsigned char adslflag;
	struct sysinfo info, *up;
	long int updays, uphours, upminutes, upsec;
	unsigned int synchronized_time[3]={0};

	d = xdsl_get_op(0);

	// check for xDSL link
	if (!d->xdsl_drv_get(RLCM_GET_LINK_SPEED, &vLs, RLCM_GET_LINK_SPEED_SIZE) || vLs.upstreamRate == 0)
		adslflag = 0;
	else
		adslflag = 1;
...
void showDSLStatistics()
{
	printf("\n");
	MSG_LINE;
#ifdef CONFIG_VDSL
	printf("                        DSL Statistics                          \n");
#else
	...
	printf("Mode\t\t\t: %s\n", strbuf);
#ifdef CONFIG_VDSL
	getAdslInfo(DSL_GET_TPS, strbuf, 256);
	printf("TPS-TC\t\t\t: %s\n", strbuf);
#endif /*CONFIG_VDSL*/
	getAdslInfo(ADSL_GET_LATENCY, strbuf, 256);
	printf("Latency\t\t\t: %s\n", strbuf);
	...
	_DSLuptime();
#ifdef CONFIG_VDSL
	getAdslInfo(DSL_GET_VECTOR_MODE, strbuf, 256);
	printf("G.Vector\t\t: %s\n", strbuf);
#endif
	printf("\n");
...
#ifdef CONFIG_VDSL
	getAdslInfo(DSL_GET_TRELLIS_DS, strbuf, 256);
	printf("Trellis\t\t\t\t\t\t: %s", strbuf);
	getAdslInfo(DSL_GET_TRELLIS_US, strbuf, 256);
	printf("\t\t%s\n", strbuf);
#endif
	...
	printf("Output Power (dBm)\t\t\t\t: %s", strbuf);
	getAdslInfo(ADSL_GET_POWER_US, strbuf, 256);
	printf("\t\t%s\n", strbuf);

	getAdslInfo(ADSL_GET_ATTRATE_DS, strbuf, 256);
	printf("Attainable Rate (Kbps)\t\t\t\t: %06s", strbuf);
	getAdslInfo(ADSL_GET_ATTRATE_US, strbuf, 256);
	printf("\t%06s\n", strbuf);

#ifdef CONFIG_VDSL
	getAdslInfo(DSL_GET_PHYR_DS, strbuf, 256);
	printf("G.INP\t\t\t\t\t\t: %s", strbuf);
	getAdslInfo(DSL_GET_PHYR_US, strbuf, 256);
	printf("\t\t%s\n", strbuf);
#endif

	getAdslInfo(ADSL_GET_RATE_DS, strbuf, 256);
	printf("Rate (Kbps)\t\t\t\t\t: %06s", strbuf);
	getAdslInfo(ADSL_GET_RATE_US, strbuf, 256);
	printf("\t%06s\n", strbuf);
...
#ifdef CONFIG_VDSL
	getAdslInfo(DSL_GET_N_DS, strbuf, 256);
	printf("N (RS codeword size)\t\t\t\t: %s", strbuf);
	getAdslInfo(DSL_GET_N_US, strbuf, 256);
	printf("\t\t%s\n", strbuf);

	getAdslInfo(DSL_GET_L_DS, strbuf, 256);
	printf("L (number of bits in DMT frame)\t\t\t: %s", strbuf);
	getAdslInfo(DSL_GET_L_US, strbuf, 256);
	printf("\t\t%s\n", strbuf);
#endif /*CONFIG_VDSL*/
...
#ifdef CONFIG_VDSL
	getAdslInfo(DSL_GET_INP_DS, strbuf, 256);
	printf("INP (DMT frame)\t\t\t\t\t: %s", strbuf);
	getAdslInfo(DSL_GET_INP_US, strbuf, 256);
	printf("\t\t%s\n", strbuf);
#endif
...
#ifdef CONFIG_VDSL
	getAdslInfo(ADSL_GET_RX_FRAMES, strbuf, 256);
	printf("OH Frame\t\t\t\t\t: %08s", strbuf);
	getAdslInfo(ADSL_GET_TX_FRAMES, strbuf, 256);
	printf("\t%08s\n", strbuf);
#endif
	...
	getAdslInfo(ADSL_GET_LOS_DS, strbuf, 256);
	printf("Total LOSS\t\t\t\t\t: %s", strbuf);
	getAdslInfo(ADSL_GET_LOS_US, strbuf, 256);
	printf("\t\t%s\n", strbuf);

#ifdef CONFIG_VDSL
	getAdslInfo(ADSL_GET_LAST_LINK_DS, strbuf, 256);
	printf("Last Link Rate\t\t\t\t\t: %s", strbuf);
	getAdslInfo(ADSL_GET_LAST_LINK_US, strbuf, 256);
	printf("\t\t%s\n", strbuf);
#endif

	getAdslInfo(ADSL_GET_FULL_INIT, strbuf, 256);
	printf("Full Init\t\t\t\t\t: %s\n", strbuf);

	getAdslInfo(ADSL_GET_FAILED_FULL_INIT, strbuf, 256);
	printf("Failed Full Init\t\t\t\t: %s\n", strbuf);

	getAdslInfo(ADSL_GET_SYNCHRONIZED_TIME, strbuf, 256);
	printf("Synchronized time (sec)\t\t\t\t: %s\n", strbuf);

	getAdslInfo(ADSL_GET_SYNCHRONIZED_NUMBER, strbuf, 256);
	printf("Synchronized number\t\t\t\t: %s\n", strbuf);
...
void showLanInterface()
{
	...
	printf("\nLAN Port Status:\n");
	for (id = 0; id < SW_LAN_PORT_NUM; id++) {
		getLANPortStatus(id, buf);
		printf("	LAN%u\t: %s\n", id + 1, buf);
	}
...
#ifdef WLAN_SUPPORT
char *encrypt_to_str(unsigned char encrypt)
{
...
void showWLAN(void)
{
	...
	printf("\n");
	MSG_LINE;
	printf("                        WLAN Settings                          \n");
	MSG_LINE;
	printf("");

	printf("\t\t\tStatus  \tEncryption\t\tSSID\n");
	...
	mib_get(MIB_WLAN_DISABLED, &wlanDisabled);
	mib_get(MIB_WLAN_ENCRYPT, &encrypt);
	mib_get(MIB_WLAN_SSID, ssid);

	MSG_LINE;
	printf("WLAN%d  %s\t\t%8.8s\t%10.10s\t\t%s\n", wlan_idx,
#ifdef WLAN_DUALBAND_CONCURRENT
			bandStr,
#else
			"\t",
#endif
			wlanDisabled ? "Disabled" : "Enabled",
			encrypt_to_str(encrypt), ssid);

#ifdef WLAN_MBSSID
	for (vap_idx = 1; vap_idx <= NUM_VWLAN_INTERFACE; vap_idx++) {
		mib_chain_get(MIB_MBSSIB_TBL, vap_idx, &Entry);
		printf("WLAN%d-VAP%d\t\t%8.8s\t%10.10s\t\t%s\n", wlan_idx, vap_idx,
				Entry.wlanDisabled ? "Disabled" : "Enabled", 
				encrypt_to_str(Entry.encrypt), Entry.ssid);
	}
#endif
...
#ifdef CONFIG_DEV_xDSL
int setDSLSettings(unsigned short mode, char olr, unsigned short profile)
{
	int xmode;
	unsigned char dsl_anxb;
...
#ifdef CONFIG_VDSL
	mib_set(MIB_VDSL2_PROFILE, &profile);
#endif
...
int addWanInterface(MIB_CE_ATM_VC_Tp pEntry)
{
	...
	do {
		...
		for (i=0; i<totalEntry; i++) {
			...
			if (MEDIA_INDEX(Entry.ifIndex) == MEDIA_ATM && mType == MEDIA_ATM
					&& Entry.vpi == pEntry->vpi && Entry.vci == pEntry->vci)
			{
				cnt++;
				pIdx = i;
			}			
		...
		if (cnt == 0)	// pvc not exists
		{
			...
			else if (pEntry->ifIndex == NA_PPP)
			{
				printf("Error: Maximum number of PPP exceeds !\n");
				return -1;
			}
			pEntry->ifIndex = TO_IFINDEX(mType, PPP_INDEX(pEntry->ifIndex), VC_INDEX(pEntry->ifIndex));
		...
		}else	// pvc exists
		{
			...
			if (Entry.cmode == CHANNEL_MODE_PPPOE && pEntry->cmode == CHANNEL_MODE_PPPOE)
			{
				...
				{	// get the pvc info.
					...
					pEntry->ifIndex = if_find_index(pEntry->cmode, ifMap);
					if (pEntry->ifIndex == NA_PPP)
					{
						printf("Error: Maximum number of PPP exceeds !\n");
						return -1;
					}
					pEntry->ifIndex &= ~DUMMY_VC_INDEX;
					pEntry->ifIndex |= VC_INDEX(Entry.ifIndex);
#ifdef _CWMP_MIB_ /*jiunming, for cwmp-tr069*/
...
int delWanInterface(char *pIfname)
{
	int ifcount = 0, idx;
	struct wstatus_info sEntry[MAX_VC_NUM + MAX_PPP_NUM];
	const size_t len = strlen(pIfname);
	MIB_CE_ATM_VC_T Entry;
#ifdef CONFIG_EXT_SWITCH
#ifdef ITF_GROUP
#ifdef WLAN_SUPPORT
	...
	for (idx = 0; idx < ifcount; idx++) {
		if (!strncmp(pIfname, sEntry[idx].ifDisplayName, len))
			break;
	}
	...
	idx = sEntry[idx].recordNum;
	if (!mib_chain_get(MIB_ATM_VC_TBL, idx, &Entry)) {
		printf("Get chain record error!\n");
		return -1;
	}
