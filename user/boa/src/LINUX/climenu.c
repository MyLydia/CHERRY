...
void setWlanAdv()
{
	...
	while (1)
	{
		...
		printf("Aggregation: %s\n", (aggre & (1<<WLAN_AGGREGATION_AMPDU))? "Enabled": "Disabled");
		...
		case 1://(1) Set
			...
			else if(hiddenSSID == 0 && wscUseVersion != 0){//enable wps when switch to broadcastssid, also check encrypt type is ok to enable wps
				if(!checkWpsDisableStatus(&Entry)){
					Entry.wsc_disabled = hiddenSSID;
				}
			}
			#endif
			#ifdef WLAN_1x
			if(hiddenSSID == 0){
				if(!Entry.wsc_disabled && is8021xEnabled(0)){//disable wsc when enable 802.1x
			...
			aggre |= (vChar&(1<<WLAN_AGGREGATION_AMSDU));
...
#ifdef CONFIG_WIFI_SIMPLE_CONFIG
#ifdef WLAN_1x
			if(!Entry.wsc_disabled && enableRS==1) //check if 802.1x enable, disable wsc.
				Entry.wsc_disabled = 1;
#endif
#endif
			mib_chain_update(MIB_MBSSIB_TBL, (void *)&Entry, 0);
...
void showWlSiteSurvey()
{
	...
	for (i=0; i<pStatus->number && pStatus->number!=0xff; i++) {
		...
		printf("%-5d%-17s%-19s%-9s%-8s%-9s%-8s\n",
			i+1, ssidbuf, tmpbuf, pBss->channel, ((pBss->capability & cIBSS) ? "Ad Hoc" : "AP"),
			((pBss->capability & cPrivacy) ? "Yes" : "No"), pBss->rssi);
		ssNum ++;
	}
}
...
void showEthWanConfiguration(MEDIA_TYPE_T m_type)
{
	...
	const char *mode;
	char *strNapt, ifname[IFNAMSIZ], ipAddr[20], ipMask[20], remoteIp[20], UserName[MAX_PPP_NAME_LEN+1], PassWord[MAX_NAME_LEN+1];
	char *temp;
...
int setNewPortMapping(int mType)
{
	...
	int total_entries, i, flags, pvcnum, min, max, num = 0, pvcidx[MAX_VC_NUM]= {0}, j = 0;
	char *temp, ifname[IFNAMSIZ], pvc[10], *aal5Encap, mode[16], ipaddr[20], remoteip[20];
	struct in_addr inAddr;
	FILE *fp;
	...
	j=0;
	for (i=PMAP_ETH0_SW0; i<=PMAP_ETH0_SW3,i<ELANVIF_NUM; i++) {
		lan_list[j].index = i;
...
//Jenny
void changeADSLSettings(XDSL_OP *d){
	...
	mib_get(MIB_ADSL_MODE, (void *)&mode);
	mib_get( MIB_DSL_ANNEX_B, (void *)&anxb);
	//if (mode & ADSL_MODE_ANXB) {
	if (anxb) {
		// annex B will not display G.lite, annex.L and annex.M, so leave it as is.
		mode &= (ADSL_MODE_GLITE|ADSL_MODE_ANXL|ADSL_MODE_ANXM);
	}
	else {
		//mode = 0;
		mode &= ADSL_MODE_ANXJ;
	...
	if (!anxb) {
		...
		if (getInputUint("ETSI (1) Enable (2) Disable:",&sel,&min,&max)==0) {
			printf("Invalid selection!\n");
			return;
		}
		if (sel==1)
	...
	if (!anxb) {
		...
		if (getInputUint("AnnexJ (1) Enable (2) Disable:",&sel,&min,&max)==0) {
			printf("Invalid selection!\n");
			return;
		}
		if (sel==1)
...
{
	// G.Vector
	if (getInputUint("G.Vector (1) Enable (2) Disable:",&sel,&min,&max)==0) {
		printf("Invalid selection!\n");
		return;
	}
	if (sel==1)
	...
	profile=0;
	sel = 0;
	if ((pfCap&VDSL2_PROFILE_8A) && getInputUint("8a (1) Enable (2) Disable:",&sel,&min,&max)==0) {
		printf("Invalid selection!\n");
		return;
	}
	if (sel==1)
	...
	sel = 0;
	if ((pfCap&VDSL2_PROFILE_8B) && getInputUint("8b (1) Enable (2) Disable:",&sel,&min,&max)==0) {
		printf("Invalid selection!\n");
		return;
	}
	if (sel==1)
	...
	sel = 0;
	if ((pfCap&VDSL2_PROFILE_8C) && getInputUint("8c (1) Enable (2) Disable:",&sel,&min,&max)==0) {
		printf("Invalid selection!\n");
		return;
	}
	if (sel==1)
	...
	sel = 0;
	if ((pfCap&VDSL2_PROFILE_8D) && getInputUint("8d (1) Enable (2) Disable:",&sel,&min,&max)==0) {
		printf("Invalid selection!\n");
		return;
	}
	if (sel==1)
	...
	sel = 0;
	if ((pfCap&VDSL2_PROFILE_12A) && getInputUint("12a (1) Enable (2) Disable:",&sel,&min,&max)==0) {
		printf("Invalid selection!\n");
		return;
	}
	if (sel==1)
	...
	sel = 0;
	if ((pfCap&VDSL2_PROFILE_12B) && getInputUint("12b (1) Enable (2) Disable:",&sel,&min,&max)==0) {
		printf("Invalid selection!\n");
		return;
	}
	if (sel==1)
	...
	sel = 0;
	if ((pfCap&VDSL2_PROFILE_17A) && getInputUint("17a (1) Enable (2) Disable:",&sel,&min,&max)==0) {
		printf("Invalid selection!\n");
		return;
	}
	if (sel==1)
	...
	sel = 0;
	if ((pfCap&VDSL2_PROFILE_30A) && getInputUint("30a (1) Enable (2) Disable:",&sel,&min,&max)==0) {
		printf("Invalid selection!\n");
		return;
	}
	if (sel==1)
	...
	sel = 0;
	if ((pfCap&VDSL2_PROFILE_35B) && getInputUint("35b (1) Enable (2) Disable:",&sel,&min,&max)==0) {
		printf("Invalid selection!\n");
		return;
	}
	if (sel==1)
		profile |= VDSL2_PROFILE_35B;
}
#endif /*CONFIG_VDSL*/
	...
	while (1)
	{
		...
		switch(sel){
			case 1:
				if (!check_access(SECURITY_SUPERUSER))
					break;
				changeADSLSettings(xdsl_get_op(0));
//Magician: Commit immediately
#ifdef COMMIT_IMMEDIATELY
				Commit();
#endif
...
//	memcpy(ifr.ifr_name, br->ifname, IFNAMSIZ);
	memcpy(ifr.ifr_name, BRIF, strlen(BRIF));
...
//Jenny
int setIPPT(struct ippt_para *para)
...
void runDiagTest(int idx)
{
	...
	pEntry = &Entry;
	if (adslflag) {
		lb5s = testOAMLoopback(pEntry, 0, 5);
		lb5e = testOAMLoopback(pEntry, 1, 5);
		lb4s = testOAMLoopback(pEntry, 0, 4);
		lb4e = testOAMLoopback(pEntry, 1, 4);
		if (cmode > 0) {
			char pingaddr[16];
...
int selTimeZone()
{
	...
	mib_get(MIB_DST_ENABLED, &dst_enabled);
	if (dst_enabled || !is_tz_dst_exist(sel_list)) {
		snprintf(tz_location,sizeof(tz_location),"%s",get_tz_location(sel_list, FOR_CLI));
		...
		tz_offset = get_format_tz_utc_offset(sel_list);
...
{
	int i;
#if defined(EMBED) && defined(CONFIG_ATM_REALTEK)
	int skfd;
	struct atmif_sioc mysio;
