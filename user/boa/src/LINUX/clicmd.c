...
static void cmdCfgDSLSetting(int cmdType, struct cmd_arg *pCmdArg)
{
...
#endif
	if(getArgValue(pCmdArg,"annexj",(void *)&intVal) == 1){	
		mode |= ADSL_MODE_ANXJ;
	}
	...
	if((pfCap&VDSL2_PROFILE_8A) && getArgValue(pCmdArg,"8a",(void *)&intVal) == 1){	
		profile |= VDSL2_PROFILE_8A;
	}
	if((pfCap&VDSL2_PROFILE_8B) && getArgValue(pCmdArg,"8b",(void *)&intVal) == 1){	
		profile |= VDSL2_PROFILE_8B;
	}
	if((pfCap&VDSL2_PROFILE_8C) && getArgValue(pCmdArg,"8c",(void *)&intVal) == 1){	
		profile |= VDSL2_PROFILE_8C;
	}
	if((pfCap&VDSL2_PROFILE_8D) && getArgValue(pCmdArg,"8d",(void *)&intVal) == 1){	
		profile |= VDSL2_PROFILE_8D;
	}
	if((pfCap&VDSL2_PROFILE_12A) && getArgValue(pCmdArg,"12a",(void *)&intVal) == 1){	
		profile |= VDSL2_PROFILE_12A;
	}
	if((pfCap&VDSL2_PROFILE_12B) && getArgValue(pCmdArg,"12b",(void *)&intVal) == 1){	
		profile |= VDSL2_PROFILE_12B;
	}
	if((pfCap&VDSL2_PROFILE_17A) && getArgValue(pCmdArg,"17a",(void *)&intVal) == 1){	
		profile |= VDSL2_PROFILE_17A;
	}
	if((pfCap&VDSL2_PROFILE_30A) && getArgValue(pCmdArg,"30a",(void *)&intVal) == 1){	
		profile |= VDSL2_PROFILE_30A;
	}
	if((pfCap&VDSL2_PROFILE_35B) && getArgValue(pCmdArg,"35b",(void *)&intVal) == 1){	
...
#ifdef EMBED
static void unlink_cli_runfile(void)
{
	if (!console_flag)
		unlink(CLI_RUNFILE);
}
