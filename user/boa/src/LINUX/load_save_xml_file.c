 ...
 */
static int skip_value_object(FILE *fp)
{
	char *pstr, *pname;
	...
	while (!feof(fp)) {
		...
		if (!strncmp(pstr, "<chain", 6)) {
			...
			if (pname == NULL) {
				printf("[%s %d] not existed !\n", __FUNCTION__, __LINE__);
				continue;
			}

			skip_value_object(fp);
		}
		//else {
	...
	while (!feof(fp)) {
		...
		if (!strncmp(pstr, "<chain", 6)) {
			...
			if (this_desc[i].name[0] == '\0') {
				...
				empty_chain = skip_value_object(fp);
				continue;
				//return error;
			}
...
static int chain_setting(FILE *fp, char *line, CONFIG_DATA_T cnf_type)
{
	...
	if (mib_chain_record_table[i].id == 0) {
		printf("%s: Invalid chain entry name: %s\n", __FUNCTION__, ptoken);
		//return error; // not found
		//put_value_object(fp, chainEntry, &root_desc, NULL);
		empty_chain = skip_value_object(fp);
		return 1;
	}
