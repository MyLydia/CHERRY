...
#if defined(CONFIG_USER_RTK_WAN_CTYPE) && !defined(CONFIG_00R0)
	// applicationtype
...
// Disable LAN DHCP
	strValue = boaGetVar(wp, "disableLanDhcp", "");
	if ( !gstrcmp(strValue, "ON"))
		entry.disableLanDhcp = 1;
	else
		entry.disableLanDhcp = 0;
...
#if defined(CONFIG_USER_RTK_WAN_CTYPE) && defined(CONFIG_RTK_L34_ENABLE) && !defined(CONFIG_00R0)
...
void writeIPEdit(request * wp, MIB_CE_ATM_VC_Tp pEntry, int index)
{
		...
		if (pEntry->cmode == CHANNEL_MODE_RT1483)
			//proto = "1483 routed";
			strcpy(proto,"1483 routed");
			
#ifdef CONFIG_ATM_CLIP
		else if (pEntry->cmode == CHANNEL_MODE_RT1577)
			//proto = "1577 routed";
			strcpy(proto,"1577 routed");
#endif
...
#ifdef CONFIG_RTK_L34_ENABLE
static int check_vlan_range(int vid, int max, int min)
{

	if(vid >= min && vid <= max){
		//AUG_PRT("%s-%d, vid=%d, max=%d, mim=%d\n",vid,max,min);
		return 1;
	}
...
static int check_vlan_reserved(int vid)
{
	...
	if(check_vlan_range(vid,fwdvlan_bind_other+DEFAULT_BIND_LAN_OFFSET,fwdvlan_bind_other))
		return 1;

	if((vid == 0) || \
	   (vid == 4095) || \
	   (vid == fwdvlan_cpu) || \
	   (vid == fwdvlan_proto_block) || \
	   (vid == fwdvlan_bind_internet) || \
	   (vid == untag_wan_vid) ||\
	   (vid == lan_vlan_id1) || \
	   (vid == lan_vlan_id2))
	   return 1;
...
/* So we let MIB_LAN_VLAN_ID1 be the same value with the first bridge WAN */
	...
	if(pEntry->vlan == 1)
	{
		...
		{
			tmpBuf+=sprintf(tmpBuf,"0,%d ,%d ,%d ,%d ,%d ,%d ,%d~%d, 4095:",fwdvlan_cpu,untag_wan_vid,lan_vlan_id1,lan_vlan_id2,fwdvlan_proto_block,fwdvlan_bind_internet,fwdvlan_bind_other,fwdvlan_bind_other+DEFAULT_BIND_LAN_OFFSET);
...
#endif
#ifdef IP_QOS
	"<th align=center width=\"3%%\">IP QoS</th>\n"
#endif
...
if(!wlanset)
			...
			boaWrite(wp, _PTI, _PME(disableLanDhcp));
#ifdef WLAN_WISP
	if(wlanset)
...
#ifdef CONFIG_00R0
	{
		...
		boaWrite(wp, _PTI _PTI _PTS _PTS,
			_PME(pppAuth), _PME(mtu), _PME(pppACName), _PME(pppServiceName));
#else
		...
		boaWrite(wp, _PTI _PTS _PTS _PTS _PTS _PTS _PTI _PTI _PTI _PTI,
			...
			_PME(dnsv6Mode));
#ifdef DUAL_STACK_LITE
		boaWrite(wp, _PTI _PTI _PTS,
			_PME(dslite_enable), _PME(dslite_aftr_mode), _PME(dslite_aftr_hostname));
#endif
#endif
#ifdef WLAN_WISP
	if(!wlanset)
...
//	checkWrite(eid, wp, argc, argv);
	...
	if ( !strcmp(name, "adslcmode") ) {
		boaWrite(wp, 
		" <th>%s:</th>"
		"<td><select size=\"1\" name=\"adslConnectionMode\" onChange=\"adslConnectionModeSelection()\">\n"
		"	  <option selected value=\"0\">1483 Bridged</option>\n"
...
void ShowMLDSetting(int eid, request * wp, int argc, char **argv)
{
#ifdef CONFIG_MLDPROXY_MULTIWAN
...
#ifdef CONFIG_00R0
void ShowRIPv2Setting(int eid, request * wp, int argc, char **argv)
{
#ifdef CONFIG_00R0
...
void ShowPPPIPSettings(int eid, request * wp, int argc, char **argv)
{
	...
	boaWrite(wp, "<div id=tbl_ppp class=\"column\">\n"
		...
		"</tr><tr>\n"
		"	<th>%s:</th>\n"
		"	<td><input type=\"password\" name=\"pppPassword\" size=\"10\" maxlength=\"%d\"></td>\n"
		"</tr>\n"
		"<tr>\n"
		"	<th>%s:</th>\n"
...
#ifdef CONFIG_00R0
void ShowPPPIPSettings_admin(int eid, request * wp, int argc, char **argv)
{
	char *key;
	int onATM;
...
#endif
					"		<th>%s:</th>\n"
					"		<td><select size=1 name=\"ctype\" onChange=\"adslConnectionTypeSelection()\">\n"
...
#ifdef CONFIG_00R0
void ShowConnectionTypeForBridge(int eid, request * wp, int argc, char **argv)
{
#ifdef CONFIG_USER_RTK_WAN_CTYPE
...
void ShowIPV6Settings(int eid, request * wp, int argc, char **argv)
{
	...
	if (vChar == 0)
		...
		boaWrite(wp, "<div id=IPv6DnsDiv style=\"display:none\">\n"
				...
				"	</tr>\n"
				"</table>\n"
				"</div>\n", multilang(LANG_PRIMARY), multilang(LANG_SECONDARY));
	...
	if (vChar == 0)
		...
		boaWrite(wp, 
				...
				"<th>%s:</th>\n"
				"<td>\n"
					"<input type=\"checkbox\" value=\"ON\" name=\"iana\" id=\"send1\">%s\n"
					"<input type=\"checkbox\" value=\"ON\" name=\"iapd\" id=\"send2\">%s\n"
				"	  </tr>\n</table>\n", multilang(LANG_REQUEST_OPTIONS),
				multilang(LANG_REQUEST_ADDRESS), multilang(LANG_REQUEST_PREFIX));
	...
	for (i=PMAP_ETH0_SW0; i<=PMAP_ETH0_SW3; i++) {
		if (i < ELANVIF_NUM) {
			...
			if (phyPortId != -1 && phyPortId == ethPhyPortId)
				boaWrite(wp, "<td style=\"display:none\"><input type=checkbox name=chkpt>LAN_%d</td>", i+1);
			else
#endif
			{
#ifndef CONFIG_GENERAL_WEB
