...
void formTcpipLanSetup(request * wp, char *path, char *query)
{
	...
	struct in_addr inIp, inMask,inStart,inEnd;
...
//star:for ip change
	...
	unsigned char cur_vChar, ip6_addr[IP6_ADDR_LEN], cur_ip6_addr[IP6_ADDR_LEN], ipv6_mode=0;
	char *strRGTmp;
#endif
	...
	mib_get( MIB_DHCP_MODE, (void *)&tmp_vChar);

	/* DHCP Client Mode don't need config ip/mask */
	if(tmp_vChar != DHCP_LAN_CLIENT){
#endif
	...
	if ( strIp[0] ) {
		...
		mib_get(MIB_DHCP_POOL_START, (void *)&inStart);
		mib_get(MIB_DHCP_POOL_END, (void *)&inEnd);  
		lip= inet_addr(strIp);
		lmask = inet_addr(strMask);

		inStart.s_addr = (lip&lmask) | ((~lmask)&inStart.s_addr);
		inEnd.s_addr = (lip&lmask) | ((~lmask)&inEnd.s_addr);
		mib_set( MIB_DHCP_POOL_START, (void *)&inStart);
		mib_set( MIB_DHCP_POOL_END, (void *)&inEnd);
...
int lan_setting(int eid, request * wp, int argc, char **argv)
{
	...
	nBytesSent += boaWrite(wp, "		<th>%s:</th>\n", multilang(LANG_IPV6_ADDRESS_MODE));
	...
	nBytesSent += boaWrite(wp, "		<td><input type=text name=lan_vlan_id2 size=15 maxlength=15 value=%s></td>\n", buf);
	nBytesSent += boaWrite(wp, "	</tr>\n");

	nBytesSent += boaWrite(wp, "<tr>\n");
	nBytesSent += boaWrite(wp, "	<th>%s:</th>\n", multilang(LANG_IP_VERSION));
	nBytesSent += boaWrite(wp, "	<td>\n");
	...
	nBytesSent += boaWrite(wp, "<tr>\n");
	nBytesSent += boaWrite(wp, "	<th>%s:</th>\n", multilang(LANG_PORT_MASK));
	nBytesSent += boaWrite(wp, "	<td>\n");
	
	for (i=PMAP_ETH0_SW0; i<=PMAP_ETH0_SW3 && i<SW_LAN_PORT_NUM; i++)
		nBytesSent += boaWrite(wp, "<input type=checkbox name=chk_port_mask2>Port %d\n", i);
