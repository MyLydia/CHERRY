...
static int getWanInfName(MIB_CE_ATM_VC_T *pEntry, char* name)
{
	...
	{//if not set by ACS. then generate automaticly.
		...
		ifGetName(pEntry->ifIndex, name, 16);
	}
	return 1;
}
...
//alex
	...
	nBytesSent += boaWrite(wp, "<th>Default QoS:&nbsp;</th>\n");
	nBytesSent += boaWrite(wp, "<td><select name=qosdmn style=\"width:100px \">\n");
	nBytesSent += boaWrite(wp, "<option value=%d>IP Pred</option>"
			"</select>\n</td>\n", PRIO_IP);
#endif
...
#else	//CONFIG_GENERAL_WEB
	nBytesSent += boaWrite(wp, "<tr>"
      	"<td align=center width=\"15%%\">Select</td>\n"
#ifdef _CWMP_MIB_
      	"<td align=center width=\"20%%\">Status</td>\n"
#endif
#ifdef NEW_IP_QOS_SUPPORT
      	"<td align=center width=\"20%%\">Src IP</td>\n"
#else
      	"<td align=center width=\"25%%\">Src IP</td>\n"
#endif
      	"<td align=center width=\"20%%\">Src Port</td>\n"
      	"<td align=center width=\"25%%\">Dst IP</td>\n"
      	"<td align=center width=\"20%%\">Dst Port</td>\n"
      	"<td align=center width=\"20%%\">Protocol</td>\n"
      	"<td align=center width=\"20%%\">Lan Port</td>\n"
#ifdef NEW_IP_QOS_SUPPORT
#ifdef QOS_DSCP_MATCH
      	"<td align=center width=\"20%%\">DSCP</td>\n"
#endif
      	//"<td align=center width=\"20%%\">802.1p</td>\n"
#endif
		);
      	nBytesSent += boaWrite(wp,
      	"<td align=center width=\"20%%\">Interface</td>\n"
      	"<td align=center width=\"20%%\">Priority</td>\n"
      	#ifdef QOS_SPEED_LIMIT_SUPPORT
	"<td align=center width=\"20%%\">Speed  Limit</td>\n"
	#endif
      	"<td align=center width=\"20%%\">IP Precd</td>\n"
      	"<td align=center width=\"20%%\">IP ToS</td>\n"
#ifdef QOS_DSCP
      	"<td align=center width=\"20%%\">IP DSCP</td>\n"
#endif
      	"<td align=center width=\"20%%\">802.1p</td>\n"
#ifdef IP_POLICY_ROUTING
      	"<td align=center width=\"20%%\">Wan IF</td>\n"
#endif
//#ifdef _CWMP_MIB_
//      	"<td align=center width=\"20%%\">Enable</td>\n"
//#endif
//      	"<td align=center width=\"15%%\">Select</td></tr>\n"));
      	"</tr>\n");
#endif	//CONFIG_GENERAL_WEB
	for (i=0; i<entryNum; i++) {
...
#else	//CONFIG_GENERAL_WEB
	nBytesSent += boaWrite(wp, "<tr>"
		"<td align=center width=\"15%%\"><input type=\"checkbox\" name=\"s%d\" value=1></td>\n"
#ifdef _CWMP_MIB_
...
int ipQosQueueList(int eid, request * wp, int argc, char **argv)
{
	...
	else if ( !strcmp(name, "RemoveAddPostSecurityFlag") )
	{
		nBytesSent += boaWrite(wp," document.qos.%s.isclick = 1;\n"
   		"postTableEncrypt(document.forms[0].postSecurityFlag, document.forms[0]);\n",multilang(LANG_REMOVEQUEUE));
		return nBytesSent;
	}
	else if (!strcmp(name, "SaveAddPostSecurityFlag") )
	{
		nBytesSent += boaWrite(wp," document.qos.%s.isclick = 1;\n"
   		"postTableEncrypt(document.forms[0].postSecurityFlag, document.forms[0]);\n",multilang(LANG_SAVEANDREBOOT));
		return nBytesSent;
	}
	else
		type = IP_QOS_QUEUE_LIST;
...
#else
				"<input class=\"link_bg\" type='submit' name=\"%s\" onClick=removeClick(this.form.removeQ) value='Remove'>\n"
				"<input class=\"link_bg\" type='submit' name=\"%s\" onClick='savRebootClick(this.form.enableQ)' value='Save'></p>\n",
#endif
	...
	if ( type == IP_QOS_QUEUE_LIST ) {
		...
		nBytesSent += boaWrite(wp, "<tr>"
			...
			"<th align=center width=\"10%%\">%s</th></tr>\n",
#endif
		...
		if ( type == IP_QOS_QUEUE_LIST ) {
			...
			nBytesSent += boaWrite(wp, "<tr>"
				...
				"<td align=center width=\"15%%\">%s</td>\n"
					"<td align=center width=\"15%%\">%s</td>\n"
					"<td align=center width=\"20%%\">%u</td>\n"
					"<td align=center width=\"15%%\">%u</td>\n"
					"<td align=center width=\"10%%\"><input type=\"checkbox\" name=\"enableQ\" value=\"%u\" %s></td>\n"
					"<td align=center width=\"10%%\"><input type=\"checkbox\" name=\"removeQ\" value=\"%u\"></td></tr>\n",
#endif
...
int confDscp(int eid, request * wp, int argc, char **argv)
{
	int nBytesSent=0;
...
#endif
#else	//CONFIG_GENERAL_WEB
	  nBytesSent += boaWrite(wp,  "\t<th>%s:&nbsp;&nbsp;</th><td><input type=\"radio\" name=dscpenable value=0 onClick=\"return dscpClick()\"></td></tr>\n", multilang(LANG_TYPE_OF_SERVIE_TOS));
#ifdef NEW_IP_QOS_SUPPORT
      nBytesSent += boaWrite(wp,  "\t<tr><th>%s:&nbsp;&nbsp;</th><td><select name=ipprio style=\"width:150px \">\n",
		      multilang(LANG_PRECEDENCE));
#else
      nBytesSent += boaWrite(wp,  "\t<tr><th>IP.Pred_Mark:&nbsp;&nbsp;</th><td><select name=ipprio>\n");
#endif
#endif	//CONFIG_GENERAL_WEB
	  ...
	  nBytesSent += boaWrite(wp,  "\t</select></td></tr>\n");
#ifdef NEW_IP_QOS_SUPPORT    
      nBytesSent += boaWrite(wp,  "\t<tr><th>TOS:&nbsp;&nbsp;</th><td><select name=tos style=\"width:150px \">\n");
#else
      nBytesSent += boaWrite(wp,  "\t<tr><th>TOS_Mark:&nbsp;&nbsp;</th><td><select name=tos>\n");
#endif
#endif	//CONFIG_GENERAL_WEB
      nBytesSent += boaWrite(wp,  "\t\t<option value=255></option>\n");
...
#else
	  nBytesSent += boaWrite(wp,  "\t<th>Diffserv Codepoint (DSCP):&nbsp;&nbsp;</th><td><input type=\"radio\" name=dscpenable value=1 onClick=\"return dscpClick()\" checked></td></tr>\n");
	  nBytesSent += boaWrite(wp,	"\t<tr><th>DSCP tag:&nbsp;&nbsp;</th><td><select name=dscp style=\"width:150px \">\n");
#endif
      nBytesSent += boaWrite(wp,  "\t\t<option value=0> </option>\n");
...
#else		
	  nBytesSent += boaWrite(wp,  "\t<th>Diffserv Codepoint (DSCP):&nbsp;&nbsp;</th><td><input type=\"radio\" name=dscpenable value=1 onClick=\"return dscpClick()\" checked></td></tr>\n");
	  nBytesSent += boaWrite(wp,	"\t<tr><th>Value:&nbsp;&nbsp;</th><td><select name=dscp>\n");
#endif
...
#else
	  nBytesSent += boaWrite(wp,  "\t<td><input type=radio name=dscpenable checked>\n");
	  nBytesSent += boaWrite(wp,	"\t<input type=radio name=dscpenable ></td>\n");
	  nBytesSent += boaWrite(wp,	"\t<td><input type=hidden name=dscp></td>\n");
#endif
      nBytesSent += boaWrite(wp,  "</tr>\n");
      #if 0
...
int diffservList(int eid, request * wp, int argc, char **argv)
{
	...
	nBytesSent += boaWrite(wp, "<tr>"
		...
		"<th align=center>PHB</th>\n"
		"<th align=center>Class Rate</th>\n"
		"<th align=center>Inf</th>\n"
		"<th align=center>BW</th>\n");
	nBytesSent += boaWrite(wp,
		"<th align=center>Latency</th>\n"
		"<th align=center>Src IP</th>\n"
		"<th align=center>Src Port</th>\n"
		"<th align=center>Dst IP</th>\n"
...
#else
		nBytesSent += boaWrite(wp, "<tr>"
			"<td align=center><input type=\"checkbox\" name=\"s%d\" value=1></td>\n"
...
#else
	nBytesSent += boaWrite(wp,  "<th>DSCP:</th>\n");
#endif
...
#else
	if(m==IPQOS_NUM_PRIOQ-1)
	      nBytesSent += boaWrite(wp, " \t\t<option selected value=%d>%s(??‰Ω?</option>\n",  IPQOS_NUM_PRIOQ-1, pPrio);
	else
	       nBytesSent += boaWrite(wp, " \t\t<option  value=%d>%s(??‰Ω?</option>\n",  IPQOS_NUM_PRIOQ-1, pPrio);
#endif
...
#else
	if(m==0)
	  nBytesSent += boaWrite(wp, "  \t\t<option selected value=0>%s(??È´?</option>\n", pPrio);
	else
	   nBytesSent += boaWrite(wp, "  \t\t<option  value=0>%s(??È´?</option>\n", pPrio);
#endif
	nBytesSent += boaWrite(wp, "  '\t</select>\n");
      nBytesSent += boaWrite(wp,  "</tr>");
   }
...
int setting_predprio(int eid, request * wp, int argc, char **argv)
{
	...
	for(i=0;i<IPQOS_NUM_PKT_PRIO;i++)
	{
		...
		nBytesSent += boaWrite(wp,	"<td align=center width=\"50%%\">%d</td>\n",i);
		nBytesSent += boaWrite(wp, "  <td align=center width=\"50%%\"><select name=\"%s\">\n",nset1ptbl);
#endif
		pPrio = prioLevel[IPQOS_NUM_PRIOQ-1];
		if(m==IPQOS_NUM_PRIOQ-1)
			nBytesSent += boaWrite(wp, " \t\t<option selected value=%d>%s(lowest)</option>\n",  IPQOS_NUM_PRIOQ-1, pPrio);
