...
int check_display(int eid, request * wp, int argc, char **argv)
{
	...
	{
#if defined(QOS_TRAFFIC_SHAPING_BY_VLANID)&&defined(CONFIG_RTK_L34_ENABLE)
		boaWrite(wp,"block");
#else
		boaWrite(wp,"none");
#endif
	...
	{
#if defined(QOS_TRAFFIC_SHAPING_BY_SSID)&&defined(CONFIG_RTK_L34_ENABLE)
		boaWrite(wp,"block");
#else
		boaWrite(wp,"none");
#endif
...
void SendWebHeadStr(int eid, request * wp, int argc, char **argv)
{
...
int CheckMenuDisplay(int eid, request * wp, int argc, char **argv)
{
	...
	if(!strcmp(name, "pon_settings"))
	{
		...
		if(!mib_get(MIB_PON_MODE, (void *)&pon_mode))
			return -1;
#if defined(CONFIG_GPON_FEATURE)
		if(pon_mode == GPON_MODE)
		{
			...
			boaWrite(wp,"{\n"
						"    name: '%s',\n"
						"    href: pageRoot + 'gpon_type.asp'\n"
						"}\n"
						",",multilang(LANG_GPON_TYPE));
#endif
...
int checkWrite(int eid, request * wp, int argc, char **argv)
{
	...
	if (!strcmp(name, "loginSelinit")) {
		...
		if ( !mib_get( MIB_GRE_ENABLE, (void *)&vChar) )
			return -1;
		if (0 == vChar)
			boaWrite(wp, "checked");
		return 0;
...
// Mason Yu. MLD Proxy
		...
		return 0;
   		...
   		if ( !mib_get( MIB_V6_ACL_CAPABILITY, (void *)&vChar) )
			return -1;
		if (0 == vChar)
			boaWrite(wp, "checked");
		return 0;
...
//xl_yue:
	...
	//for web log
		...
		if (!mib_get(MIB_SAMBA_SECURITY_ENABLE, &vChar))
			return -1;
		...
		boaWrite(wp,"<tr>"
			...
			"</td>"
			"</tr>", (0 == vChar)?"checked":"", multilang(LANG_DISABLE), (1 == vChar)?"checked":"", multilang(LANG_ENABLE));
#endif
		return 0;
	}
#endif
		...
		mib_get( MIB_DSL_ANNEX_B, (void *)&vChar);
	...
	if (!strcmp(name, "syslog-mode")) {
		...
		return 0;
   		...
   		if ( !mib_get( CWMP_FLAG2, (void *)&vUInt) )
			return -1;
		...
		if (!mib_get(CWMP_ACL_ENABLE, &vChar))
			return -1;
		if (0 == vChar)
			boaWrite(wp, "checked");
		return 0;
...
/* add by yq_zhou 09.2.02 add sagem logo for 11n*/
...
#endif
#if defined(CONFIG_RTK_L34_ENABLE) && defined (CONFIG_IPV6)
	...
	// 0 - Mosocow, 1-Sankt-Peterburg, 2-Chelyabinsk
		...
		if (!mib_get(MIB_ROSTELECOM_PROVINCES, (void *)&vChar))
			return -1;
		if (vChar==0)
			boaWrite(wp,"1");
		else
	...
	else if (!strcmp(name, "vlan_mapping_tabel")) {
		int i = 0, j = 0;
		...
		for (i = 0; i < ELANVIF_NUM; i++) {
			...
			for (j = 0; j < MAX_PAIR; j++) {
				boaWrite(wp, ",[");
				boaWrite(wp, "%d,%d", vid_pair[j].vid_a, vid_pair[j].vid_b);
				boaWrite(wp, "]");
			}
...
#endif
#endif
	else if(!strcmp(name,"98F_qos_mod"))
	{
#ifdef CONFIG_RTK_DEV_AP
		boaWrite(wp,"1");
#else
	...
	else if(!strcmp(name,"98F_qos_del_comment_start"))
	{
#ifdef CONFIG_RTK_DEV_AP
		boaWrite(wp, "<!--");
#else
	...
	else if(!strcmp(name,"98F_qos_del_comment_end"))
	{
#ifdef CONFIG_RTK_DEV_AP
		boaWrite(wp, "-->");
#else
		...
		if(sscanf( buff,
			...
			if( len == 0 && (strcmp(devname, "lo") !=0) ) {
				if (getInFlags( devname, &flags) != 1)
					continue;
				if (flags & IFF_POINTOPOINT) { // if point-to-point, we display devname.
