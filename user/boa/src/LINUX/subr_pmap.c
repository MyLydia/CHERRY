...
// bit[19:15]/5b as LAN_MARK
//pol_mark[ETH0_SW0] = "0x1"  which means:  mark the eth0_sw0 as 0x00008000/0x000f8080
//2014/2/27 modify mask 0x000f8000 to 0x000f8080 because route don't need VLAN header.
//          clear bit 7 VLAN Enable ,set in ebtables.
const char*  pol_mark[] = {"0x00008000/0x000f8080",	/* 1 */
			"0x00010000/0x000f8080",	/* 2 */
			"0x00018000/0x000f8080",	/* 3 */
			"0x00020000/0x000f8080",	/* 4 */
			"0x00028000/0x000f8080",	/* 5 */
			"0x00030000/0x000f8080",	/* 6 */
			"0x00038000/0x000f8080",	/* 7 */
			"0x00040000/0x000f8080",	/* 8 */
			"0x00048000/0x000f8080",	/* 9 */
			"0x00050000/0x000f8080",	/* 10*/
			"0x00058000/0x000f8080",	/* 11*/
			"0x00060000/0x000f8080",	/* 12*/
			"0x00068000/0x000f8080",	/* 13*/
			"0x00070000/0x000f8080",""};	/* 14*/
		...
		if(BIT_IS_SET(fgroup, j))
		{
			...
			if(BIT_IS_SET(itfGroup, j)) {
				...
				if ((pEntry->cmode == CHANNEL_MODE_BRIDGE)) {
					...
					#ifdef _LINUX_3_18_
					va_cmd(IPTABLES, 12, 1, (char *)FW_ADD, (char *)PORTMAP_IPTBL,
						"-m", "physdev", "--physdev-in", ELANVIF[j], "-p", (char *)ARG_UDP,
						(char *)FW_DPORT, (char *)PORT_DHCP, "-j", (char *)FW_DROP);
					#else
					va_cmd(IPTABLES, 10, 1, (char *)FW_ADD, (char *)PORTMAP_IPTBL,
						(char *)ARG_I, ELANVIF[j], "-p", (char *)ARG_UDP,
						(char *)FW_DPORT, (char *)PORT_DHCP, "-j", (char *)FW_DROP);
					#endif
#endif
#ifdef CONFIG_USER_IGMPPROXY
					// If this LAN interface maps to bridge-wan, don't go igmp proxy so we drop IGMP packets on INPUT.
					// ebtables -A portmapping_igmp -i eth0.2 -p IPv4 --ip-proto 2 -j DROP
					va_cmd(EBTABLES, 10, 1, "-A", PORTMAP_IGMP, "-i", ELANVIF[j], "-p", "IPv4", "--ip-proto", "2", "-j", "DROP");
#endif
				}
			}
		}
	    ...
	    if(BIT_IS_SET(fgroup, j))
	    {
			...
			if(BIT_IS_SET(itfGroup, j)) {
				...
				if ((pEntry->cmode == CHANNEL_MODE_BRIDGE)) {
					...
					#ifdef _LINUX_3_18_
					va_cmd(IPTABLES, 12, 1, (char *)FW_ADD, (char *)PORTMAP_IPTBL,
						"-m", "physdev", "--physdev-in", wlan[j-PMAP_WLAN0], "-p", (char *)ARG_UDP,
						(char *)FW_DPORT, (char *)PORT_DHCP, "-j", (char *)FW_DROP);
					#else
					va_cmd(IPTABLES, 10, 1, (char *)FW_ADD, (char *)PORTMAP_IPTBL,
...
#ifdef CONFIG_USER_IGMPPROXY
					// If this LAN interface maps to bridge-wan, don't go igmp proxy so we drop IGMP packets on INPUT.
					va_cmd(EBTABLES, 10, 1, "-A", PORTMAP_IGMP, "-i", wlan[j-PMAP_WLAN0], "-p", "IPv4", "--ip-proto", "2", "-j", "DROP");
#endif
				}
			}
		}
...
#endif
		if(isFileExist("/proc/rg/slaveWifiBind"))
				...
				if(!iptbl_done_once)
				{
					...
					for(j = PMAP_ETH0_SW0; j <= PMAP_ETH0_SW3 && j < ELANVIF_NUM; ++j)
					{
						...
						if (pbEntry.pb_mode == PORT_BAESD_MODE)
							{
#endif
								va_cmd(IPTABLES, 12, 1, "-t", "mangle", "-A", LANIF_MARK,
									"-m", "physdev", "--physdev-in", ELANVIF[j],
									"-j", "MARK",
									"--set-mark", pol_mark[j]);
#ifdef CONFIG_IPV6
...
#endif
								va_cmd(IPTABLES, 12, 1, "-t", "mangle", "-A", LANIF_MARK,
									"-m", "physdev", "--physdev-in", wlan[j-PMAP_WLAN0],
									"-j", "MARK",
									"--set-mark", pol_mark[j]);
#ifdef CONFIG_RTL867X_VLAN_MAPPING
							}
...
#ifdef CONFIG_USER_IGMPPROXY
	// ebtables -F portmapping_igmp
	va_cmd(EBTABLES, 2, 1, "-F", (char *)PORTMAP_IGMP);
#endif
...
//--------------------------------------------------------------------------------------------
					...
					if(port <= PMAP_ETH0_SW3 && port < SW_LAN_PORT_NUM)
						...
						RG_add_vlanBinding(&vc_Entry,k,vid_pair[k].vid_a,port);						
#endif
					RTK_RG_VLAN_Binding_MC_DS_Rule_Config(0);
#endif
