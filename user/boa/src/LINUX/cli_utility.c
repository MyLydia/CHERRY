	...
	if (isConnect) {
		if (!havePPP) {
			...
			if (Entry.cmode == CHANNEL_MODE_PPPOE)	// PPPoE
			{
				...
				snprintf(msg.data, BUF_SIZE, "spppctl add %u pppoe %s username %s password %s gw 1",
					PPP_INDEX(Entry.ifIndex), ifGetName(PHY_INTF(Entry.ifIndex), devname, sizeof(devname)), Entry.pppUsername, Entry.pppPassword);
			}
			else	// PPPoA
			{
