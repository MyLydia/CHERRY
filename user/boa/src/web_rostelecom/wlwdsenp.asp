...
<body>
<blockquote>
<h2 class="page_title"><% multilang(LANG_WDS_SECURITY_SETTINGS); %> <% if (getIndex("wlan_num")>1) write("-wlan"+(checkWrite("wlan_idx")+1));
%></h2>

<form action=/boaform/formWdsEncrypt method=POST name="formWdsEncrypt">
<table>
  <tr><td><font size=2>
    <% multilang(LANG_THIS_PAGE_ALLOWS_YOU_SETUP_THE_WLAN_SECURITY_FOR_WDS_WHEN_ENABLED_YOU_MUST_MAKE_SURE_EACH_WDS_DEVICE_HAS_ADOPTED_THE_SAME_ENCRYPTION_ALGORITHM_AND_KEY); %>
  </font></td></tr>
...
<table>
  ...
  <tr>
     ...
     <input type="submit" value="<% multilang(LANG_APPLY_CHANGES); %>" name="save" onClick="return saveChanges(document.formWdsEncrypt)">&nbsp;&nbsp;
     <input type="button" value="<% multilang(LANG_CLOSE); %>" name="close" OnClick=window.close()>&nbsp;&nbsp;
