...
{
	if (ip.value=="") {
		alert("<% multilang(LANG_INVALID_IPV4_ADDR_SHOULD_NOT_EMPTY); %>"); 
	...
	if( IsLoopBackIP( ip.value)==1 ) {
		alert("<% multilang(LANG_INVALID_IPV4_ADDR); %>");
	...
	if ( !checkDigitRange(ip.value,1,0,255) ) {
		alert("<% multilang(LANG_INVALID_IPV4_ADDR_1ST_DIGIT); %>");
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
	if ( !checkDigitRange(ip.value,2,0,255) ) {
		alert("<% multilang(LANG_INVALID_IPV4_ADDR_2ND_DIGIT); %>");
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
	if ( !checkDigitRange(ip.value,3,0,255) ) {
		alert("<% multilang(LANG_INVALID_IPV4_ADDR_3RD_DIGIT); %>");
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
	if ( !checkDigitRange(ip.value,4,1,254) ) {
		alert("<% multilang(LANG_INVALID_IPV4_ADDR_4TH_DIGIT); %>");
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
...
function checkNetmask(netmask, checkEmpty)
{
	...
	if (validateKey(netmask.value) == 0) {
		alert("<% multilang(LANG_INVALID_IPV4_SUBNET_SHOULD_BE_DECIMAL_NUM); %>");
		netmask.value = netmask.defaultValue;
		netmask.focus();
		return false;
	}
	...
	for (i=1; i<=4; i++) {
		...
		if( !(d==0 || d==128 || d==192 || d==224 || d==240 || d==248 || d==252 || d==254 || d==255 )) {
			alert("<% multilang(LANG_INVALID_IPV4_SUBNET_DIGIT); %>");
			netmask.focus();
			return false;
		}
	}
...
function checkMac(macAddr, checkEmpty)
{
	...
	if (checkEmpty == 1 && macAddr.value.length == 0) {
		alert("<% multilang(LANG_INVALID_MAC_ADDR_SHOULD_NOT_EMPTY); %>");
		return false;
	}
	...
	for (i=0; i<macAddr.value.length; i++) {
		if ((macAddr.value.charAt(i) >= '0' && macAddr.value.charAt(i) <= '9') ||
			...
			(macAddr.value.charAt(i) >= 'A' && macAddr.value.charAt(i) <= 'F') )
			continue;
		alert("<% multilang(LANG_INVALID_MAC_ADDR_SHOULD_BE); %>");
		macAddr.focus();
		return false;
	}
	return true;
...
{
	if ( !confirm('<% multilang(LANG_CONFIRM_DELETE); %>') ) {
		return false;
	}
...
function alertInvalid(fieldname, fieldvalue, additional)
{
	if (additional == undefined)
		alert (fieldname + " " + fieldvalue +  "<% multilang(LANG_INVALID); %>");
	else
		alert (fieldname + " " + fieldvalue + "<% multilang(LANG_INVALID); %>" + additional + ".");
}
...
function checkAdminPasswordValueComplex(str)
{
	var hasDigit = 0;
	var hasLetter = 0;
...
function checkSuperAdminPasswordValueComplex(str)
{
	var hasDigit = 0;
	var hasSmallLetter = 0;
	var hasBigLetter = 0;
	...
	if(hasDigit<3 ||hasSmallLetter<3 || hasBigLetter<3)
		return false;
