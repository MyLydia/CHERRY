...
function applyCheck()
{
	...
	if ( document.ethwan.adslConnectionMode.value == 2 ) {
		if (document.ethwan.mru.value > 1492 || document.ethwan.mru.value < 65 || document.ethwan.mru.value =="") {
			alert('<% multilang(LANG_STRMRUERR); %>');
			document.ethwan.mru.focus();
			return false;
		}
	...
	//Disable need to remove, otherwise the value will not pass to backend
	document.ethwan.ctype.disabled = false;
...
function connectionTypeChangeDisplay()
{
	with (document.forms[0]) {
		if( adslConnectionMode.value!=0){
			...
			document.getElementById('connectionTypeForBridge').style.display = 'block';
...
function adslConnectionModeSelection(isLinkChanged)
{
	...
	//document.ethwan.ctype.disabled = true;
...
function on_linkchange(itlk)
{
	...
	with ( document.forms[0] )
	{
		if(itlk == null)
		{
			//Allow admin to add INTERNET type only
			document.getElementById("div_wanpage").style.display = "block";
			...
			//checkbox
			naptEnabled.checked = vlan.checked = qosEnabled.checked = igmpEnabled.checked = ripv2Enabled.checked = false;
		...
		else
		{
			...
			document.getElementById("div_wanpage").style.display = "block";
			if(ctype.value == 2) //INTERNET only, allow user to apply
				document.getElementById("delete").style.display = "block";
			else //INTERNET with others , don't allow user to apply
				document.getElementById("delete").style.display = "none";
			//document.ethwan.ctype.disabled = true;
...
<table border=0 width="800" cellspacing=4 cellpadding=0>
	...
	<tr>
		...
		<!--	<b><% multilang(LANG_ENABLE_QOS); %>: </b> -->
			<input type="hidden" name="qosEnabled" size="2" maxlength="2" value="ON" >
		</font></td>
	</tr>
	<tr>
...
<BR>
<input type="hidden" value="/admin/multi_wan_generic_admin.asp" name="submit-url">
...
<table>
   <tr>
     ...
     <td>
		<input type="submit" value="<% multilang(LANG_DELETE); %>" name="delete" id="delete" onClick="return deleteCheck()">
     </td>
   </tr>
</table>
