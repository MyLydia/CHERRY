...
function showDhcpSvr(ip)
{
	...
	if (document.dhcpd.dhcpdenable[0].checked == true)
		...
		document.getElementById('displayDhcpSvr').innerHTML=
			...
			'<table border=0 width="500" cellspacing=4 cellpadding=0>'+
			'<tr>'+
			'<td width="40%"><font size=2><b>DHCP <% multilang(LANG_SERVER); %> <% multilang(LANG_IP_ADDRESS); %>:</b></td>'+
		...
		html +=
			...
			'<input type="button" value="<% multilang(LANG_DEVICE); %> <% multilang(LANG_IP_RANGE); %>" name="clientIpTbl" onClick="clientIpClick(\'/devicetbl.asp\')">';
