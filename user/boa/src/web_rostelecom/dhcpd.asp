...
function showDhcpSvr(ip)
{
	...
	if (document.dhcpd.dhcpdenable[0].checked == true)
		...
		html+=
			'<input type="button" value="<% multilang(LANG_SHOW_CLIENT); %>" name="dhcpClientTbl" onClick="dhcpTblClick(\'/admin/dhcptbl.asp\')" >'+
			'</td>'+
			'</tr>';
		...
		html +=
			...
			'<input type="button" value="<% multilang(LANG_PORT_BASED_FILTER); %>" name="macIpTbl" onClick="macIpClick(\'/admin/portBaseFilterDhcp.asp\')">'+
			'<input type="button" value="<% multilang(LANG_MAC_BASED_ASSIGNMENT); %>" name="macIpTbl" onClick="macIpClick(\'/admin/macIptbl.asp\')">';
...
<h2 class="page_title">DHCP <% multilang(LANG_SETTINGS); %></h2>

<form action=/boaform/admin/formDhcpServer method=POST name="dhcpd">
...
<table>
  ...
  <tr><td ID="displayDhcpSvr"></td></tr>
   ...
   <br>
      <input type="hidden" value="/admin/dhcpd.asp" name="submit-url">
