...
<body>
<blockquote>
<h2 class="page_title"><% multilang(LANG_LAN_INTERFACE_SETTINGS); %></h2>

<form action=/boaform/admin/formTcpipLanSetup method=POST name="tcpip">
<table>
  <tr><td>
...
<table>
  ...
  <tr>
      <th><% multilang(LANG_INTERFACE); %><% multilang(LANG_NAME); %>:</th>
      <td>br0</td>
  </tr>
  <tr>
      <th><% multilang(LANG_IP_ADDRESS); %>:</th>
      <td><input type="text" name="ip" size="15" maxlength="15" value=<% getInfo("lan-ip"); %>></td>
  </tr>
  <tr>
      <th><% multilang(LANG_SUBNET_MASK); %>:</th>
      <td><input type="text" name="mask" size="15" maxlength="15" value="<% getInfo("lan-subnet"); %>"></td>
  </tr>
	<tr id=tr_ipv6_mode1 <% checkWrite("rg_only_function"); %>>
		<th><% multilang(LANG_IPV6_ADDRESS_MODE); %>:</th>
		<td>
  ...
  <tr id=tr_ipv6_addr1 <% checkWrite("rg_only_function"); %>>
      ...
      <td><input type="text" name="ipv6_addr1" size="30" maxlength="60" value=0></td>
  </tr>
  <tr id=tr_ipv6_prefix1 <% checkWrite("rg_only_function"); %>>
      <th><% multilang(LANG_IPV6_PREFIX_LENGTH); %>:</th>
      <td><input type="text" name="ipv6_prefix1" size="5" maxlength="5" value=0></td>
  </tr>
  	<!-- Hide VID setting temporarily -->
	<tr style="display:none">
		<th><% multilang(LANG_VLAN_ID); %>:</th>
		<td><input type="text" name="lan_vlan_id1" size="15" maxlength="15" value="<% getInfo("lan_vlan_id1"); %>"></td>
	</tr>
	<tr <% checkWrite("rg_only_function"); %>>
		<th><% multilang(LANG_IP_VERSION); %>:</th>
		<td>
  ...
  <br>
      ...
      <input type="hidden" value="/admin/tcpiplan.asp" name="submit-url">
