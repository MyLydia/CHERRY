		...
		*/
		...
		switch(rules[i].ipqos_rule_type)
		{
			...
			case 3:
				...
				break;
			case 4:
				cell.innerHTML =  "<b>Connect Type<br></b>";
				cell.innerHTML += rules[i].conntypeStr;	
				break;
			case 5:
				cell.innerHTML =  "<b>Application<br></b>";
				cell.innerHTML += rules[i].applicationStr;	
				break;				
			case 6:
	...
	<blockquote>
		<DIV align="left" style="padding-left:20px; padding-top:5px;">
			<form id="form" action="/boaform/admin/formQosRule" method="post">
				<h2 class="page_title">QoS <% multilang(LANG_CLASSIFICATION); %></h2>				
				<div class="tip" style="width:90% ">
					<% multilang(LANG_PAGE_DESC_CLASSICY_QOS_RULE); %><font color="red">(<% multilang(LANG_PAGE_DESC_CLASSICY_QOS_RULE_EXTRA); %>)</font>
				</div>
				<table>
				...
				<table class="flat" id="lstrc" border="1" cellpadding="0" cellspacing="1">
				   <tr class="hdb" align="center" nowrap bgcolor="#CCCCCC"><font size=2>
						<td colspan="2">&nbsp;</td>
						<td colspan="3"><b><% multilang(LANG_MARK); %></b></td>
						<td colspan="3" width="300"><b><% multilang(LANG_CLASSIFICATION_RULES); %></b></td>
						<td colspan="3">&nbsp;</td>
					</tr>
					<strong>
				   <tr class="hdb" align="center" nowrap>
						<td><font size=2><% multilang(LANG_ID); %></td>
						<td><font size=2><% multilang(LANG_NAME); %></td>
						<td><font size=2>Order</td>
						<!--<td><font size=2><% multilang(LANG_VLAN_ID); %></td>-->
						<td><font size=2>DSCP <% multilang(LANG_MARK); %></td>
						<td><font size=2><% multilang(LANG_802_1P); %></td>
						               
						<td><font size=2> <% multilang(LANG_QUEUE); %></td>
						<td><font size=2> <% multilang(LANG_WANIF); %></td>
						<td><font size=2 width="280"><% multilang(LANG_RULE_DETAIL); %></td>					
						<td><font size=2><% multilang(LANG_DELETE); %></td>
						<td><font size=2><% multilang(LANG_EDIT); %></td>
						<td><font size=2>&nbsp;</td>
				...
				<br>
				<input type="button" class="button" onClick="location.href='net_qos_cls_edit.asp';" value="<% multilang(LANG_ADD); %>">
