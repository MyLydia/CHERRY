...
<title><% multilang(LANG_WLAN_BASIC_SETTINGS); %></title>
...
//					  [36,40,44,48,52,56,60,64,132,136,140,149,153,157,161,165],
  					  [36,38,40,42,44,46,48,52,54,56,58,60,62,64,132,134,136,138,140,142,149,151,153,155,157,159,161,165],
...
function showChannel2G(bound_40, band)
{
	...
	document.wlanSetup.chan.length=0;
	idx=0;
	document.wlanSetup.chan.options[idx] = new Option("<% multilang(LANG_AUTO); %>", 0, false, false);
	if (0 == wlan_channel) {
		document.wlanSetup.chan.selectedIndex = 0;
	}
	idx++;	
...
function showchannelbound_updated(band)
{
	...
	if(wlan_support_11ncoexist){
		...
		if(band==75)
			document.wlanSetup.chanwid.options[idx++] = new Option("20/40/80MHz", "4", false, false);
	}
...
<body>
<blockquote>
<h2 class="page_title"><% multilang(LANG_WLAN_BASIC_SETTINGS); %></h2>

<form action=/boaform/admin/formWlanSetup method=POST name="wlanSetup">
<table>
  <tr><td><font size=2>
 <% multilang(LANG_PAGE_DESC_WLAN_BASIC_SETTING); %>
  </font></td></tr>
...
<table width="400">
  <tr>
      <th colspan=2>
   	<input type="checkbox" name="wlanDisabled" value=<% wlanStatus(); %> 
   	 ONCLICK=updateState(document.wlanSetup)>&nbsp;&nbsp;<% multilang(LANG_DISABLE_WLAN_INTERFACE); %>
     </th>
  </tr>

  <tr>
      <th><% multilang(LANG_BAND); %>:</th>
      <td><select size=1 name=band onChange="updateChan(); Set_SSIDbyBand(document.wlanSetup);">
      		<% checkWrite("wlband"); %>
	 </select>
  ...
  <tr>
      ...
      <td><select size="1" name="mode" onChange="updateMode()">
      		<% checkWrite("wlmode"); %>
      		</select>
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<input type="button" value="<% multilang(LANG_MULTIPLE_AP); %>" name="multipleAP" onClick="showMultipleAP(document.wlanSetup, '/admin/wlmultipleap.asp')">
      </td>
  </tr>
  
  <tr>
      <th><% multilang(LANG_SSID); %>:</th>
      <td><input type=text name=ssid size="25" maxlength="32" value="<% getInfo("ssid"); %>">
      </td>
  </tr>
  </table>
  ...
  <table width="400">
  <tr>
      <th><% multilang(LANG_CHANNEL_WIDTH); %>:</th>
      <td><select size="1" name="chanwid" onChange="updateChan1()">
            	<% checkWrite("wlchanwid"); %>
      		</select>
      </td>
  </tr>
  <tr <% checkWrite("wlan_qtn_hidden_function"); %> >
      <th><% multilang(LANG_CONTROL_SIDEBAND); %>:</th>
  ...
  <tr <% checkWrite("wlan_qtn_hidden_function"); %>>
      <th><% multilang(LANG_RADIO_POWER); %>:</th>
  ...
  <tr>
      ...
      value="<% multilang(LANG_SHOW_ACTIVE_WLAN_CLIENTS); %>" name="showMac" 
  ...
  <tr id="wifi_regdomain_demo" style="display:none">
      <th><% multilang(LANG_REGDOMAIN); %>:</th>
      <td><select size="1" name="regdomain_demo">
  ...
  <tr>
     ...
     <input type="submit" value="<% multilang(LANG_APPLY_CHANGES); %>" 
