...
static inline int get_alias_hash_value(char *file);
...
int translate_uri(request * req)
{
  ...
  while (current) {
	...
	if (strstr(req->request_uri,"share.js") 
#if defined(CONFIG_GENERAL_WEB)
		|| strstr(req->request_uri,"adminMenu.js") || strstr(req->request_uri,"userMenu.js")
#endif
